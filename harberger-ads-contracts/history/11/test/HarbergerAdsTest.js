...
contract("HarbergerAds",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractHarbergerAds = await HarbergerAds.new(contractERC20test.address,0,17,{from:accounts[0]});
    if(trace) console.log('SUCESSO: HarbergerAds.new(contractERC20test.address,0,17,{from:accounts[0]}');
  });
  ...
  it('Should execute addProperty(uint256)', async () => {
    let result = await contractHarbergerAds.addProperty(65,{from: accounts[0]});
  });
  ...
  it('Should execute buy(uint256,uint256,uint256) WHEN seller!=msg.sender', async () => {
    let result = await contractHarbergerAds.buy(95, 28, 95,{from: accounts[6]});
  });
  it('Should execute buy(uint256,uint256,uint256) WHEN seller==msg.sender', async () => {
    let result = await contractHarbergerAds.buy(1532892062, 1331007097, 3,{from: accounts[2]});
  });
  ...
  it('Should execute changeRecipient(uint256,address)', async () => {
    let result = await contractHarbergerAds.changeRecipient(26, accounts[6],{from: accounts[0]});
  });
});
