...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(256,accounts[5],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(256,accounts[5],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(255,accounts[2],contractAtlantisToken.address,29,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+626,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+626+255,3,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(255,accounts[2],contractAtlantisToken.address,29,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+626,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+626+255,3,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(6,accounts[0],contractAtlantisToken.address,1532892064,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[2], 2014223714,{from: accounts[0]});
  });
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[2], 1532892063,{from: accounts[0]}),'revert');
  });
});
