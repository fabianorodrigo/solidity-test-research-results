...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(1532892062,accounts[7],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(1532892062,accounts[7],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(95,accounts[8],contractAtlantisToken.address,1338,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+742,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+742+113,254,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(95,accounts[8],contractAtlantisToken.address,1338,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+742,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+742+113,254,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(2,accounts[8],contractAtlantisToken.address,2014223716,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[7], 2014223716,{from: accounts[0]});
  });
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[7], 2014223715,{from: accounts[0]}),'revert');
  });
});
