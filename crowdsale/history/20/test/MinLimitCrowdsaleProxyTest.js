...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(66,accounts[3],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(66,accounts[3],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(1337,accounts[8],contractAtlantisToken.address,256,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42+749,2014223716,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(1337,accounts[8],contractAtlantisToken.address,256,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42+749,2014223716,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(255,accounts[0],contractAtlantisToken.address,1532892064,{from:accounts[0]});
});
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[4], 1532892063,{from: accounts[0]}),'revert');
  });
});
