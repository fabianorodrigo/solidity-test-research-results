...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(95,accounts[0],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(95,accounts[0],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(3,accounts[5],contractAtlantisToken.address,1,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+499,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+499+52,10001,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(3,accounts[5],contractAtlantisToken.address,1,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+499,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+499+52,10001,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(1336,accounts[4],contractAtlantisToken.address,1,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[5], 65,{from: accounts[0]});
  });
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: beneficiary != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase("0x0000000000000000000000000000000000000000", 65,{from: accounts[0]}),'revert');
  });
