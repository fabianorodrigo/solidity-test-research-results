...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(1,accounts[6],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(1,accounts[6],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(2014223715,accounts[6],contractAtlantisToken.address,65,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+995,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+995+33,5,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(2014223715,accounts[6],contractAtlantisToken.address,65,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+995,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+995+33,5,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(10001,accounts[1],contractAtlantisToken.address,96,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[6], 96,{from: accounts[0]});
  });
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: beneficiary != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase("0x0000000000000000000000000000000000000000", 96,{from: accounts[0]}),'revert');
  });
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[6], 95,{from: accounts[0]}),'revert');
  });
});
