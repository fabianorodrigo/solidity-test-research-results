...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(501,accounts[5],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(501,accounts[5],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(3,accounts[8],contractAtlantisToken.address,4,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+633,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+633+998,9999,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(3,accounts[8],contractAtlantisToken.address,4,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+633,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+633+998,9999,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(2014223716,accounts[4],contractAtlantisToken.address,257,{from:accounts[0]});
});
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[0], 256,{from: accounts[0]}),'revert');
  });
});
