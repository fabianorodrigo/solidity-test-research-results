...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(1338,accounts[4],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(1338,accounts[4],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(97,accounts[4],contractAtlantisToken.address,5,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+314,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+314+922,1532892064,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(97,accounts[4],contractAtlantisToken.address,5,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+314,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+314+922,1532892064,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(97,accounts[5],contractAtlantisToken.address,95,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[1], 97,{from: accounts[0]});
  });
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: beneficiary != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase("0x0000000000000000000000000000000000000000", 97,{from: accounts[0]}),'revert');
  });
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[1], 94,{from: accounts[0]}),'revert');
  });
});
