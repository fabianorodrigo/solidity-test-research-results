...
contract("GolemNetworkTokenBatching",(accounts)=>{
  ...
  it('Should execute batchTransfer(bytes32[],uint64) WHEN (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp>=closureTime', async () => {
    let result = await contractGolemNetworkTokenBatching.batchTransfer([[5,182,110,119,94,82,148,231,172,174,181,152,200,102,167,11,57,145,70,200,196,223,125,68,195,3,39,178,18,75,130,239]], (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp-987,{from: accounts[0]});
  });
  it('Should fail batchTransfer(bytes32[],uint64) when NOT comply with: (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp >= closureTime', async () => {
    let result = await truffleAssert.fails(contractGolemNetworkTokenBatching.batchTransfer([[5,182,110,119,94,82,148,231,172,174,181,152,200,102,167,11,57,145,70,200,196,223,125,68,195,3,39,178,18,75,130,239]], (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+1,{from: accounts[0]}),'revert');
  });
  it('Should execute transferAndCall(address,uint256,bytes) WHEN to!=0x0000000000000000000000000000000000000000,value<=balances', async () => {
    let result = await contractGolemNetworkTokenBatching.transferAndCall(accounts[1], 160, [97,147,194,151,153,147,242,116,47,57,221,70,102,212,242,218,50,77,175,127,115,29,156,221,152,189,177,236,185,69,203,66],{from: accounts[0]});
  });
  it('Should fail transferAndCall(address,uint256,bytes) when NOT comply with: to != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractGolemNetworkTokenBatching.transferAndCall("0x0000000000000000000000000000000000000000", 160, [97,147,194,151,153,147,242,116,47,57,221,70,102,212,242,218,50,77,175,127,115,29,156,221,152,189,177,236,185,69,203,66],{from: accounts[0]}),'revert');
  });
});
