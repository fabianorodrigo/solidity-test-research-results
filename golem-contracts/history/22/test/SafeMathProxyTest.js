...
contract("contractProxySafeMath",(accounts)=>{
  ...
  it('Should execute testmul(uint256,uint256) WHEN a!=0', async () => {
    let result = await contractProxySafeMath.testmul(257, 160,{from: accounts[0]});
  });
  it('Should execute testdiv(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testdiv(1, 83,{from: accounts[0]});
  });
  it('Should execute testsub(uint256,uint256) WHEN b<=a', async () => {
    let result = await contractProxySafeMath.testsub(19, 16,{from: accounts[0]});
  });
  ...
  it('Should execute testadd(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testadd(256, 160,{from: accounts[0]});
  });
});
