...
contract("GolemNetworkToken",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN senderBalance>=_value,_value>0', async () => {
    let result = await contractGolemNetworkToken.transfer(accounts[3], 10,{from: accounts[0]});
  });
  it('Should execute transfer(address,uint256) WHEN senderBalance<_value', async () => {
    let result = await contractGolemNetworkToken.transfer(accounts[7], 159,{from: accounts[0]});
  });
  ...
  it('Should execute migrate(uint256) WHEN migrationAgent!=0x0000000000000000000000000000000000000000,_value!=0,_value<=balances', async () => {
    await contractGolemNetworkToken.setMigrationAgent(accounts[5],{from:accounts[4]});
    let result = await contractGolemNetworkToken.migrate(16,{from: accounts[4]});
  });
  it('Should fail migrate(uint256) when NOT comply with: migrationAgent != 0x0000000000000000000000000000000000000000', async () => {
    ...
    let result = await truffleAssert.fails(contractGolemNetworkToken.migrate(16,{from: accounts[4]}),'revert');
  });
  ...
  it('Should execute setMigrationAgent(address) WHEN migrationAgent==0x0000000000000000000000000000000000000000,msg.sender==migrationMaster', async () => {
    let result = await contractGolemNetworkToken.setMigrationAgent(accounts[2],{from: accounts[4]});
  });
  it('Should fail setMigrationAgent(address) when NOT comply with: msg.sender == migrationMaster', async () => {
    let result = await truffleAssert.fails(contractGolemNetworkToken.setMigrationAgent(accounts[2],{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute create() WHEN funding==true,(await web3.eth.getBlockNumber())>=fundingStartBlock,(await web3.eth.getBlockNumber())<=fundingEndBlock,msg.value!=0', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[3],accounts[9],(await web3.eth.getBlockNumber())+561,(await web3.eth.getBlockNumber())+561+8,{from:accounts[0]});
    let result = await localGolemNetworkToken.create({from: accounts[0],value:10});
  });
  it('Should fail create() when NOT comply with: funding == true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[3],accounts[9],(await web3.eth.getBlockNumber())+561,(await web3.eth.getBlockNumber())+561+8,{from:accounts[0]});
    await localGolemNetworkToken.finalize({from:accounts[0],value:10});
    let result = await truffleAssert.fails(localGolemNetworkToken.create({from: accounts[0],value:10}),'revert');
  });
  ...
  it('Should execute refund() WHEN funding==true,(await web3.eth.getBlockNumber())>fundingEndBlock,totalTokens<tokenCreationMin,gntValue!=0,FunctionCall==true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[4],accounts[0],(await web3.eth.getBlockNumber())+929,(await web3.eth.getBlockNumber())+929+796,{from:accounts[0]});
