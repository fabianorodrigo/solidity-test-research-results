...
contract("GolemNetworkToken",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN senderBalance>=_value,_value>0', async () => {
    let result = await contractGolemNetworkToken.transfer(accounts[5], 21,{from: accounts[0]});
  });
  ...
  it('Should execute migrate(uint256) WHEN migrationAgent!=0x0000000000000000000000000000000000000000,_value!=0,_value<=balances', async () => {
    await contractGolemNetworkToken.setMigrationAgent(accounts[1],{from:accounts[4]});
    let result = await contractGolemNetworkToken.migrate(160,{from: accounts[4]});
  });
  it('Should fail migrate(uint256) when NOT comply with: migrationAgent != 0x0000000000000000000000000000000000000000', async () => {
    ...
    let result = await truffleAssert.fails(contractGolemNetworkToken.migrate(160,{from: accounts[4]}),'revert');
  });
  ...
  it('Should execute setMigrationAgent(address) WHEN migrationAgent==0x0000000000000000000000000000000000000000,msg.sender==migrationMaster', async () => {
    let result = await contractGolemNetworkToken.setMigrationAgent(accounts[5],{from: accounts[4]});
  });
  ...
  it('Should execute create() WHEN funding==true,(await web3.eth.getBlockNumber())>=fundingStartBlock,(await web3.eth.getBlockNumber())<=fundingEndBlock,msg.value!=0', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[4],accounts[1],(await web3.eth.getBlockNumber())+532,(await web3.eth.getBlockNumber())+532+695,{from:accounts[0]});
    let result = await localGolemNetworkToken.create({from: accounts[0],value:3});
  });
  it('Should fail create() when NOT comply with: funding == true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[4],accounts[1],(await web3.eth.getBlockNumber())+532,(await web3.eth.getBlockNumber())+532+695,{from:accounts[0]});
    await localGolemNetworkToken.finalize({from:accounts[0],value:3});
    let result = await truffleAssert.fails(localGolemNetworkToken.create({from: accounts[0],value:3}),'revert');
  });
  ...
  it('Should execute refund() WHEN funding==true,(await web3.eth.getBlockNumber())>fundingEndBlock,totalTokens<tokenCreationMin,gntValue!=0,FunctionCall==true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[4],accounts[6],(await web3.eth.getBlockNumber())+195,(await web3.eth.getBlockNumber())+195+106,{from:accounts[0]});
