...
contract("GolemNetworkToken",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN senderBalance>=_value,_value>0', async () => {
    let result = await contractGolemNetworkToken.transfer(accounts[0], 10000,{from: accounts[0]});
  });
  ...
  it('Should execute migrate(uint256) WHEN migrationAgent!=0x0000000000000000000000000000000000000000,_value!=0,_value<=balances', async () => {
    ...
    let result = await contractGolemNetworkToken.migrate(17,{from: accounts[3]});
  });
  it('Should fail migrate(uint256) when NOT comply with: migrationAgent != 0x0000000000000000000000000000000000000000', async () => {
    ...
    let result = await truffleAssert.fails(contractGolemNetworkToken.migrate(17,{from: accounts[3]}),'revert');
  });
  ...
  it('Should execute setMigrationMaster(address) WHEN msg.sender==migrationMaster,_master!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractGolemNetworkToken.setMigrationMaster(accounts[9],{from: accounts[3]});
  });
  it('Should fail setMigrationMaster(address) when NOT comply with: msg.sender == migrationMaster', async () => {
    let result = await truffleAssert.fails(contractGolemNetworkToken.setMigrationMaster(accounts[9],{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute create() WHEN funding==true,(await web3.eth.getBlockNumber())>=fundingStartBlock,(await web3.eth.getBlockNumber())<=fundingEndBlock,msg.value!=0', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[6],accounts[9],(await web3.eth.getBlockNumber())+904,(await web3.eth.getBlockNumber())+904+80,{from:accounts[0]});
    let result = await localGolemNetworkToken.create({from: accounts[0],value:9});
  });
  it('Should fail create() when NOT comply with: funding == true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[6],accounts[9],(await web3.eth.getBlockNumber())+904,(await web3.eth.getBlockNumber())+904+80,{from:accounts[0]});
    await localGolemNetworkToken.finalize({from:accounts[0],value:9});
    let result = await truffleAssert.fails(localGolemNetworkToken.create({from: accounts[0],value:9}),'revert');
  });
  ...
  it('Should execute refund() WHEN funding==true,(await web3.eth.getBlockNumber())>fundingEndBlock,totalTokens<tokenCreationMin,gntValue!=0,FunctionCall==true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[7],accounts[3],(await web3.eth.getBlockNumber())+755,(await web3.eth.getBlockNumber())+755+875,{from:accounts[0]});
