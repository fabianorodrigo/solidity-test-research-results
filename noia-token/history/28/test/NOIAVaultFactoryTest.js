...
contract("NOIAVaultFactory",(accounts)=>{
  ...
  it('Should execute createVault(address,uint256)', async () => {
    let result = await contractNOIAVaultFactory.createVault(accounts[1], 66,{from: accounts[0]});
  });
  it('Should execute unlockableBalanceOf(address)', async () => {
    let result = await contractNOIAVaultFactory.unlockableBalanceOf(accounts[1],{from: accounts[0]});
  });
  ...
  it('Should execute getVaultsCount(address)', async () => {
    let result = await contractNOIAVaultFactory.getVaultsCount(accounts[0],{from: accounts[0]});
  });
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN balance>=amount', async () => {
    let result = await contractNOIAVaultFactory.recoverTokens(contractNOIAToken.address, accounts[6], 3,{from: accounts[0]});
  });
});
