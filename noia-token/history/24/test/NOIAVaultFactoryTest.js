...
contract("NOIAVaultFactory",(accounts)=>{
  ...
  it('Should execute createVault(address,uint256)', async () => {
    let result = await contractNOIAVaultFactory.createVault(accounts[6], 4,{from: accounts[0]});
  });
  ...
  it('Should execute totalBalanceOf(address)', async () => {
    let result = await contractNOIAVaultFactory.totalBalanceOf(accounts[5],{from: accounts[0]});
  });
  ...
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN balance>=amount', async () => {
    let result = await contractNOIAVaultFactory.recoverTokens(contractNOIAToken.address, accounts[5], 96,{from: accounts[0]});
  });
});
