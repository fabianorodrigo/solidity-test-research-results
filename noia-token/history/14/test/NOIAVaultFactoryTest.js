...
contract("NOIAVaultFactory",(accounts)=>{
  ...
  it('Should execute createVault(address,uint256)', async () => {
    let result = await contractNOIAVaultFactory.createVault(accounts[4], 999999999,{from: accounts[0]});
  });
  ...
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN balance>=amount', async () => {
    let result = await contractNOIAVaultFactory.recoverTokens(contractNOIAToken.address, accounts[3], 256,{from: accounts[0]});
  });
});
