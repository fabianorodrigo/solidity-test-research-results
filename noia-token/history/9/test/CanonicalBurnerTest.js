...
contract("CanonicalBurner",(accounts)=>{
  ...
  it('Should execute tokensReceived(address,address,uint256) WHEN msg.sender==token', async () => {
    let result = await contractCanonicalBurner.tokensReceived(accounts[4], accounts[8], 9999,{from: contractNOIAToken.address});
  });
  it('Should fail tokensReceived(address,address,uint256) when NOT comply with: msg.sender == token', async () => {
    let result = await truffleAssert.fails(contractCanonicalBurner.tokensReceived(accounts[4], accounts[8], 9999,{from: accounts[9]}),'revert');
  });
});
