...
contract("contractProxyBaseSwaps",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractBaseSwaps = await BaseSwaps.new(accounts[8],accounts[3],accounts[2],4038714809,254,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+114,{from:accounts[0]});
    if(trace) console.log('SUCESSO: BaseSwaps.new(accounts[8],accounts[3],accounts[2],4038714809,254,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+114,{from:accounts[0]}');
      ProxyBaseSwaps.link("SafeMath",contractSafeMath.address);
    contractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[8],accounts[3],accounts[2],4038714809,254,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+114,{from:accounts[0]});
});
  ...
  it('Should fail test_refund(address) when NOT comply with: isSwapped != true', async () => {
    ...
    let result = await truffleAssert.fails(contractProxyBaseSwaps.test_refund(accounts[1],{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute test_sendTokens(address,address,uint) WHEN _token==0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens("0x0000000000000000000000000000000000000000", accounts[3], 11,{from: accounts[0]});
  });
  it('Should execute test_sendTokens(address,address,uint) WHEN _token!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens(accounts[8], accounts[9], 10001,{from: accounts[0]});
  });
  it('Should execute test_removeInvestor(address[],address)', async () => {
    let result = await contractProxyBaseSwaps.test_removeInvestor([accounts[0],accounts[0],accounts[9],accounts[2],accounts[8],accounts[3]], accounts[3],{from: accounts[0]});
  });
  it('Should execute test_depositEther(uint) WHEN (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_depositEther(256,{from: accounts[0]});
  });
  it('Should execute test_depositTokens(address,uint) WHEN baseAddress==_token,quoteAddress==_token,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_depositTokens(accounts[8], 257,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall!=true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[1], accounts[6], 4,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall==true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[6], accounts[6], 5,{from: accounts[0]});
  });
  it('Should execute test_isInvestor(address,address)', async () => {
    let result = await contractProxyBaseSwaps.test_isInvestor(accounts[0], accounts[3],{from: accounts[0]});
  });
});
