...
contract("contractProxyBaseSwaps",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[7],accounts[0],accounts[9],2014223716,4038714809,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+967,{from:accounts[0]});
});
  ...
  it('Should fail test_refund(address) when NOT comply with: isSwapped != true', async () => {
    let localcontractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[5],accounts[5],"0x0000000000000000000000000000000000000000",97,255,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+640,{from:accounts[0]});
  ...
  it('Should fail test_swap() when NOT comply with: isSwapped != true', async () => {
    let localcontractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[6],accounts[7],"0x0000000000000000000000000000000000000000",1532892063,1532892063,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+429,{from:accounts[0]});
    await localcontractProxyBaseSwaps.sendTransaction({from:accounts[0]});
  ...
  it('Should execute test_sendTokens(address,address,uint) WHEN _token!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens(accounts[9], accounts[5], 1,{from: accounts[0]});
  });
  it('Should execute test_removeInvestor(address[],address)', async () => {
    let result = await contractProxyBaseSwaps.test_removeInvestor([accounts[7],accounts[8],accounts[0]], accounts[7],{from: accounts[0]});
  });
  ...
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall!=true,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[6], accounts[0], 257,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall==true,baseAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[4], accounts[4], 257,{from: accounts[0]});
  });
