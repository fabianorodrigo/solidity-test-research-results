...
contract("contractProxyERC20BondingToken",(accounts)=>{
  ...
  it('Should execute test_curvedMint(uint256) WHEN msg.gasprice<=gasPrice,amount>0,msg.sender!=0', async () => {
    let result = await contractProxyERC20BondingToken.test_curvedMint(95,{from: accounts[6]});
  });
  it('Should fail test_curvedMint(uint256) when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractProxyERC20BondingToken.test_curvedMint(95,{from: accounts[6]}),'revert');
  });
  ...
  it('Should fail test_curvedMint(uint256) when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractProxyERC20BondingToken.test_curvedMint(95,{from: 0}),'revert');
  });
  it('Should execute test_curvedBurn(uint256)', async () => {
    let result = await contractProxyERC20BondingToken.test_curvedBurn(93,{from: accounts[0]});
  });
});
