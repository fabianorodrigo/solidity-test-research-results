...
contract("ERC20BondingToken",(accounts)=>{
  ...
  it('Should execute initialize(ERC20,uint256,uint256,uint32,uint256) WHEN initialized!=true', async () => {
    let result = await contractERC20BondingToken.methods["initialize(address,uint256,uint256,uint32,uint256)"](contractEthBondingToken.address, 114, 37, 38, 41,{from: accounts[0]});
  });
  ...
  it('Should execute calculateCurvedBurnReturn(uint256)', async () => {
    let result = await contractERC20BondingToken.calculateCurvedBurnReturn(82,{from: accounts[0]});
  });
});
