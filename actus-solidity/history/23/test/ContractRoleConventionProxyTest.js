...
contract("contractProxyContractRoleConvention",(accounts)=>{
    let contractProxyContractRoleConvention = null;
  let trace = false;
  ...
  beforeEach(async () => {
    ...
    if(trace) console.log('SUCESSO: BusinessDayConvention.new({from: accounts[0]}');
      contractProxyContractRoleConvention = await ProxyContractRoleConvention.new({ from: accounts[0] });
});
  
  it('Should execute testroleSign(ContractRole) WHEN contractRole==Definitions.ContractRole.RPA', async () => {
    let result = await contractProxyContractRoleConvention.testroleSign(0,{from: accounts[0]});
  });
  it('Should execute testroleSign(ContractRole) WHEN contractRole!=Definitions.ContractRole.RPA', async () => {
    let result = await contractProxyContractRoleConvention.testroleSign(7,{from: accounts[0]});
  });
  it('Should execute testroleSign(ContractRole) WHEN contractRole==Definitions.ContractRole.RPL', async () => {
    let result = await contractProxyContractRoleConvention.testroleSign(1,{from: accounts[0]});
  });
  it('Should execute testroleSign(ContractRole) WHEN contractRole!=Definitions.ContractRole.RPL,contractRole!=Definitions.ContractRole.RPA', async () => {
    let result = await contractProxyContractRoleConvention.testroleSign(3,{from: accounts[0]});
  });
});
