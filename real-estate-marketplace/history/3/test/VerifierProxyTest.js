...
contract("contractProxyVerifier",(accounts)=>{
  ...
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall!=true', async () => {
    let result = await contractProxyVerifier.testverify([15,2014223715,6,47,32,20], {"A": {"X": 22,"Y": 2014223716},"B": {"X": [9,55],"Y": [27,256]},"C": {"X": 103,"Y": 1023}},{from: accounts[0]});
  });
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall==true', async () => {
    let result = await contractProxyVerifier.testverify([30,71,0,30,101,162,33,162,256], {"A": {"X": 30,"Y": 46},"B": {"X": [20,32],"Y": [7,98]},"C": {"X": 63,"Y": 98}},{from: accounts[0]});
  });
});
