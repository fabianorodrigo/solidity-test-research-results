...
contract("contractProxyVerifier",(accounts)=>{
  ...
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall!=true', async () => {
    let result = await contractProxyVerifier.testverify([1532892064,64,87,45,27,7,98], {"A": {"X": 97,"Y": 3},"B": {"X": [65,1532892063],"Y": [5,2]},"C": {"X": 17,"Y": 255}},{from: accounts[0]});
  });
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall==true', async () => {
    let result = await contractProxyVerifier.testverify([60,1025,55,257,70,63], {"A": {"X": 1025,"Y": 31},"B": {"X": [11,129],"Y": [2,49]},"C": {"X": 98,"Y": 20}},{from: accounts[0]});
  });
});
