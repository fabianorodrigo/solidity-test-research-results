...
contract("contractProxyVerifier",(accounts)=>{
  ...
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall!=true', async () => {
    let result = await contractProxyVerifier.testverify([1023], {"A": {"X": 6,"Y": 128},"B": {"X": [45,60],"Y": [257,1]},"C": {"X": 199999,"Y": 128}},{from: accounts[0]});
  });
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall==true', async () => {
    let result = await contractProxyVerifier.testverify([58,1532892064], {"A": {"X": 1,"Y": 11},"B": {"X": [61,69],"Y": [61,254]},"C": {"X": 25,"Y": 64}},{from: accounts[0]});
  });
});
