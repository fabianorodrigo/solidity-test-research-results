...
contract("Token",(accounts)=>{
  ...
  it('Should execute mint(address,string) WHEN msg.sender==owner,FunctionCall!=true,_to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractToken.mint(accounts[9], "tumpin by jeffrey alan scudder, 2017, 32 editions",{from: accounts[0]});
  });
  it('Should fail mint(address,string) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractToken.mint(accounts[9], "tumpin by jeffrey alan scudder, 2017, 32 editions",{from: accounts[9]}),'revert');
  });
  it('Should fail mint(address,string) when NOT comply with: _to != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractToken.mint("0x0000000000000000000000000000000000000000", "tumpin by jeffrey alan scudder, 2017, 32 editions",{from: accounts[0]}),'revert');
  });
