...
contract("contractProxyLeftGalleryToken",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractERC721Token = await ERC721Token.new("superuser","autobreeder (lite) by harm van den dorpel, 2016, 100 editions",{from:accounts[0]});
    if(trace) console.log('SUCESSO: ERC721Token.new("superuser","autobreeder (lite) by harm van den dorpel, 2016, 100 editions",{from:accounts[0]}');
    ...
    contractToken = await Token.new("ivqwrh","lgu",4,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Token.new("ivqwrh","lgu",4,{from:accounts[0]}');
  ...
  it('Should execute test_setTokenURI(uint256,string)', async () => {
    let result = await contractProxyLeftGalleryToken.test_setTokenURI(1331007098, "\x19Ethereum Signed Message:\n32",{from: accounts[0]});
  });
  it('Should execute testaddTokenTo(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testaddTokenTo(accounts[0], 4038714809,{from: accounts[0]});
  });
  it('Should execute testremoveTokenFrom(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testremoveTokenFrom(accounts[9], 97,{from: accounts[0]});
  });
  ...
  it('Should execute test_burn(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.test_burn(accounts[9], 3,{from: accounts[0]});
  });
});
