...
contract("Token",(accounts)=>{
  ...
  it('Should execute mint(address,string) WHEN msg.sender==owner,FunctionCall!=true,_to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractToken.mint(accounts[0], "bubamara ocr by adriana ramić, 2018, 100 editions",{from: accounts[0]});
  });
  it('Should fail mint(address,string) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractToken.mint(accounts[0], "bubamara ocr by adriana ramić, 2018, 100 editions",{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute burn(uint256) WHEN msg.sender==owner', async () => {
    let result = await contractToken.burn(4038714809,{from: accounts[0]});
  });
  it('Should fail burn(uint256) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractToken.burn(4038714809,{from: accounts[9]}),'revert');
  });
});
