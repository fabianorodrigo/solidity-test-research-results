...
contract("LeftGalleryToken",(accounts)=>{
  ...
  it('Should execute tokenByIndex(uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenByIndex(29,{from: accounts[0]});
  });
  it('Should execute createWork(address,string) WHEN msg.sender==owner,_to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractLeftGalleryToken.createWork(accounts[5], "lgbcs",{from: accounts[0]});
  });
  it('Should fail createWork(address,string) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractLeftGalleryToken.createWork(accounts[5], "lgbcs",{from: accounts[9]}),'revert');
  });
  it('Should fail createWork(address,string) when NOT comply with: _to != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractLeftGalleryToken.createWork("0x0000000000000000000000000000000000000000", "lgbcs",{from: accounts[0]}),'revert');
  });
});
