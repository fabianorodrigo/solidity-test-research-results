...
contract("contractProxyLeftGalleryToken",(accounts)=>{
  ...
  it('Should execute test_setTokenURI(uint256,string)', async () => {
    let result = await contractProxyLeftGalleryToken.test_setTokenURI(4, "bouncer",{from: accounts[0]});
  });
  it('Should execute testaddTokenTo(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testaddTokenTo(accounts[7], 1,{from: accounts[0]});
  });
  it('Should execute testremoveTokenFrom(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testremoveTokenFrom(accounts[7], 97,{from: accounts[0]});
  });
  ...
  it('Should execute test_burn(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.test_burn(accounts[2], 4038714809,{from: accounts[0]});
  });
});
