...
contract("LeftGalleryToken",(accounts)=>{
  ...
  it('Should execute tokenURI(uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenURI(4,{from: accounts[0]});
  });
  it('Should execute tokenOfOwnerByIndex(address,uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenOfOwnerByIndex(accounts[4], 31,{from: accounts[0]});
  });
  ...
  it('Should execute tokenByIndex(uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenByIndex(1331007097,{from: accounts[0]});
  });
  it('Should execute createWork(address,string) WHEN msg.sender==owner,_to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractLeftGalleryToken.createWork(accounts[4], "lgfile",{from: accounts[0]});
  });
  it('Should fail createWork(address,string) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractLeftGalleryToken.createWork(accounts[4], "lgfile",{from: accounts[9]}),'revert');
  });
