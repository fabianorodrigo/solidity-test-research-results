...
contract("MembershipVerificationToken",(accounts)=>{
  ...
  it('Should fail requestMembership(uint[]) when NOT comply with: _attributeIndexes.length == attributeNames.length', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.requestMembership([97],{from: accounts[0]}),'revert');
  });
  ...
  it('Should fail assignTo(address,uint[]) when NOT comply with: _attributeIndexes.length == attributeNames.length', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.assignTo(accounts[0], [64],{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute addAttributeSet(bytes32,bytes32[])', async () => {
    let result = await contractMembershipVerificationToken.addAttributeSet([72,237,177,114,21,127,98,173,40,51,184,150,148,163,220,165,4,241,23,113,131,237,131,88,92,56,80,136,28,70,164,103], [[255,200,214,202,36,227,73,136,43,12,0,236,237,122,217,73,226,122,60,81,110,82,156,41,230,153,25,33,199,208,158,111],[135,117,16,17,154,239,174,147,77,214,144,161,200,230,70,54,84,15,0,134,23,189,223,4,21,72,31,125,165,176,219,217],[4,228,28,82,88,140,121,99,58,166,193,181,21,27,44,45,150,46,104,138,67,97,81,252,116,237,134,122,108,232,169,130],[11,27,140,176,224,246,140,14,197,83,16,183,27,180,133,190,250,139,56,58,92,122,51,149,42,201,233,244,201,100,72,148],[38,160,224,171,140,187,7,105,234,86,66,64,207,66,102,152,164,149,207,98,42,4,124,32,239,239,221,194,79,75,191,22],[98,191,234,109,64,7,242,178,120,169,48,111,120,211,148,51,7,128,220,253,201,228,121,165,194,21,249,225,110,195,190,142],[252,159,16,84,69,26,151,178,111,229,1,223,200,4,212,229,190,164,196,53,248,133,161,141,5,34,194,251,78,101,86,109]],{from: accounts[0]});
  });
  it('Should execute modifyAttributeByIndex(address,uint,uint) WHEN msg.sender==_owner', async () => {
    let result = await contractMembershipVerificationToken.modifyAttributeByIndex(accounts[6], 3, 64,{from: accounts[0]});
  });
  it('Should fail modifyAttributeByIndex(address,uint,uint) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.modifyAttributeByIndex(accounts[6], 3, 64,{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute getAttributeByIndex(address,uint)', async () => {
    let result = await contractMembershipVerificationToken.getAttributeByIndex(accounts[4], 27,{from: accounts[0]});
  });
