...
contract("MembershipVerificationToken",(accounts)=>{
  ...
  it('Should fail requestMembership(uint[]) when NOT comply with: _attributeIndexes.length == attributeNames.length', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.requestMembership([1532892063],{from: accounts[0]}),'revert');
  });
  ...
  it('Should fail assignTo(address,uint[]) when NOT comply with: _attributeIndexes.length == attributeNames.length', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.assignTo(accounts[8], [9999],{from: accounts[0]}),'revert');
  });
  it('Should execute revokeFrom(address) WHEN msg.sender==_owner,_from!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractMembershipVerificationToken.revokeFrom(accounts[1],{from: accounts[0]});
  });
  it('Should fail revokeFrom(address) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.revokeFrom(accounts[1],{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute addAttributeSet(bytes32,bytes32[])', async () => {
    let result = await contractMembershipVerificationToken.addAttributeSet([22,72,108,61,233,156,223,53,115,81,234,83,10,27,165,81,36,139,124,31,38,83,178,123,182,105,110,191,150,4,197,145], [[144,44,84,220,89,136,193,216,107,8,105,39,21,71,26,214,4,21,166,20,32,127,55,232,191,78,40,170,24,149,134,169],[128,117,160,17,26,121,5,51,252,113,142,152,160,213,6,158,39,18,209,206,207,117,70,92,19,33,131,129,97,246,98,75],[30,180,130,234,66,89,49,50,104,162,0,27,204,123,12,117,194,121,233,86,188,138,43,97,80,193,12,197,176,214,1,46],[22,78,141,52,37,81,120,248,179,138,64,176,92,190,192,201,214,255,228,183,92,163,46,13,57,88,135,211,27,46,61,249],[150,167,232,117,84,127,65,238,100,246,64,169,134,95,24,33,189,204,107,163,30,176,154,61,82,48,204,244,214,182,15,155],[122,208,32,105,105,246,220,31,184,34,61,160,18,197,112,168,85,35,247,162,192,158,253,72,107,14,171,54,56,50,80,42],[153,26,200,172,244,224,240,150,94,92,155,16,98,226,212,47,164,46,168,203,135,219,72,113,48,139,174,145,73,74,195,254]],{from: accounts[0]});
  });
  it('Should execute modifyAttributeByIndex(address,uint,uint) WHEN msg.sender==_owner', async () => {
    let result = await contractMembershipVerificationToken.modifyAttributeByIndex(accounts[9], 0, 1532892062,{from: accounts[0]});
  });
  it('Should fail modifyAttributeByIndex(address,uint,uint) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.modifyAttributeByIndex(accounts[9], 0, 1532892062,{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute getAttributeExhaustiveCollection(uint)', async () => {
    let result = await contractMembershipVerificationToken.getAttributeExhaustiveCollection(96,{from: accounts[0]});
  });
  it('Should execute getAttributeByIndex(address,uint)', async () => {
    let result = await contractMembershipVerificationToken.getAttributeByIndex(accounts[8], 1337,{from: accounts[0]});
  });
