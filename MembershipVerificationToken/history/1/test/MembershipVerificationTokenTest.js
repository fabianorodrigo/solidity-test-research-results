...
contract("MembershipVerificationToken",(accounts)=>{
  ...
  it('Should fail assignTo(address,uint[]) when NOT comply with: _attributeIndexes.length == attributeNames.length', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.assignTo(accounts[3], [9999],{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute addAttributeSet(bytes32,bytes32[])', async () => {
    let result = await contractMembershipVerificationToken.addAttributeSet([61,69,149,119,81,226,62,159,72,59,130,215,86,120,116,124,155,125,108,45,68,70,106,22,231,69,122,179,118,22,233,135], [[135,6,242,176,89,219,120,187,151,109,73,232,113,187,179,227,108,20,71,68,244,92,208,64,128,197,34,20,146,131,28,138],[141,170,119,93,27,140,83,47,18,85,217,239,166,147,250,43,252,199,29,72,80,163,51,237,127,131,202,68,15,190,112,7]],{from: accounts[0]});
  });
  it('Should execute modifyAttributeByIndex(address,uint,uint) WHEN msg.sender==_owner', async () => {
    let result = await contractMembershipVerificationToken.modifyAttributeByIndex(accounts[1], 64, 4,{from: accounts[0]});
  });
  it('Should fail modifyAttributeByIndex(address,uint,uint) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractMembershipVerificationToken.modifyAttributeByIndex(accounts[1], 64, 4,{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute getAttributeExhaustiveCollection(uint)', async () => {
    let result = await contractMembershipVerificationToken.getAttributeExhaustiveCollection(1338,{from: accounts[0]});
  });
  it('Should execute getAttributeByIndex(address,uint)', async () => {
    let result = await contractMembershipVerificationToken.getAttributeByIndex(accounts[8], 97,{from: accounts[0]});
  });
