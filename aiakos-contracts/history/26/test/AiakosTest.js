...
contract("Aiakos",(accounts)=>{
  ...
  it('Should execute deployRelease(string,bytes32) WHEN MemberAccess.Identifier!=true', async () => {
    let result = await contractAiakos.deployRelease("minter", [109,92,14,171,105,168,60,44,33,88,161,119,91,106,164,94,43,253,211,24,246,114,178,75,118,113,65,26,170,24,39,84],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN MemberAccess.Identifier==true', async () => {
    let result = await contractAiakos.deployRelease("minter", [171,105,157,241,49,171,53,190,88,193,32,111,97,203,75,249,91,184,9,135,128,134,165,43,198,46,25,38,134,146,232,247],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN approvalGranted==true', async () => {
    let result = await contractAiakos.deployRelease("bouncer", [122,173,28,43,252,100,218,120,87,76,52,128,36,247,227,53,173,97,214,49,107,135,135,188,141,73,90,80,131,118,154,143],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN approvalGranted!=true', async () => {
    let result = await contractAiakos.deployRelease("\x19Ethereum Signed Message:\n32", [194,169,1,119,139,176,249,212,4,248,149,130,100,84,47,162,84,208,247,7,183,174,173,211,3,127,79,207,225,164,57,23],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN releaseApproved==true', async () => {
    let result = await contractAiakos.deployRelease("Example", [158,61,35,9,232,156,92,41,50,215,95,64,62,210,100,25,60,86,216,22,92,194,22,221,196,240,28,125,179,79,12,42],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN releaseApproved!=true,approvalGranted!=true', async () => {
    let result = await contractAiakos.deployRelease("superuser", [113,153,215,57,136,10,247,70,157,214,249,20,224,197,25,179,249,116,192,129,61,148,176,227,232,77,156,129,131,170,130,52],{from: accounts[0]});
  });
  it('Should execute checkRelease(string,bytes32) WHEN _sha256Hash!=0x0,MemberAccess.Identifier==true', async () => {
    let result = await contractAiakos.checkRelease("initialized", [242,94,116,12,149,14,229,10,187,52,31,3,11,225,58,115,162,16,172,37,173,131,253,176,122,200,221,113,94,90,231,205],{from: accounts[0]});
  });
  ...
  it('Should execute getReleaseInfo(string)', async () => {
    let result = await contractAiakos.getReleaseInfo("RevertWithReason",{from: accounts[0]});
  });
