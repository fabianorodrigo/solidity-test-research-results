...
contract("RedeemableToken",(accounts)=>{
  ...
  it('Should execute redeemAllTokens(address) WHEN msg.sender==redemption,msg.sender==_owner,balance>0', async () => {
    ...
    let result = await contractRedeemableToken.redeemAllTokens(accounts[7],{from: accounts[6]});
  });
  ...
  it('Should execute redeemPartialTokens(address,uint256) WHEN msg.sender==redemption,msg.sender==_owner', async () => {
    ...
    let result = await contractRedeemableToken.redeemPartialTokens(accounts[8], 1338,{from: accounts[0]});
  });
  it('Should fail redeemPartialTokens(address,uint256) when NOT comply with: msg.sender == redemption', async () => {
    await contractRedeemableToken.setRedemption(accounts[5],{from:accounts[0]});
    let result = await truffleAssert.fails(contractRedeemableToken.redeemPartialTokens(accounts[8], 1338,{from: accounts[9]}),'revert');
  });
