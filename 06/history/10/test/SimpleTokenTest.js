...
contract("SimpleToken",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractSimpleCrowdfund = await SimpleCrowdfund.new(17,5,{from:accounts[0]});
    if(trace) console.log('SUCESSO: SimpleCrowdfund.new(17,5,{from:accounts[0]}');
  });
  
  it('Should execute transferFrom(address,address,uint256)', async () => {
    let result = await contractSimpleToken.transferFrom(accounts[0], accounts[7], 6,{from: accounts[0]});
  });
  it('Should execute approve(address,uint256)', async () => {
    let result = await contractSimpleToken.approve(accounts[3], 6,{from: accounts[0]});
  });
  it('Should execute allowance(address,address)', async () => {
    let result = await contractSimpleToken.allowance(accounts[3], accounts[7],{from: accounts[0]});
  });
  ...
  it('Should execute transfer(address,uint256) WHEN _to!=0x0000000000000000000000000000000000000000,_value<=balances', async () => {
    let result = await contractSimpleToken.transfer(accounts[7], 7,{from: accounts[0]});
  });
  ...
  it('Should execute mint(address,uint256) WHEN msg.sender==_owner,totalSupply<maxSupply', async () => {
    let result = await contractSimpleToken.mint(accounts[1], 750,{from: accounts[0]});
  });
  it('Should fail mint(address,uint256) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractSimpleToken.mint(accounts[1], 750,{from: accounts[9]}),'revert');
  });
