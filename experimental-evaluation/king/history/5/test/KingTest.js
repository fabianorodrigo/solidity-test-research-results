...
contract("King",(accounts)=>{
  ...
  it('Should execute tokenFallback(address,uint256,bytes) WHEN value>=PLAY_COST', async () => {
    let result = await contractKing.tokenFallback(accounts[7], 758, [205,192,175,76,228,204,24,35,249,118,211,69,228,173,183,218,252,170,122,187,22,103,252,199,7,4,126,189,217,154,112,224],{from: accounts[0]});
  });
  it('Should fail tokenFallback(address,uint256,bytes) when NOT comply with: value >= PLAY_COST', async () => {
    let result = await truffleAssert.fails(contractKing.tokenFallback(accounts[7], 757, [205,192,175,76,228,204,24,35,249,118,211,69,228,173,183,218,252,170,122,187,22,103,252,199,7,4,126,189,217,154,112,224],{from: accounts[0]}),'revert');
  });
