...
contract("contractProxyContractDefaultConvention",(accounts)=>{
    let contractProxyContractDefaultConvention = null;
  let trace = false;
  ...
  beforeEach(async () => {
    ...
    if(trace) console.log('SUCESSO: BusinessDayConvention.new({from: accounts[0]}');
      contractProxyContractDefaultConvention = await ProxyContractDefaultConvention.new({ from: accounts[0] });
});
  
  it('Should execute testperformanceIndicator(ContractStatus) WHEN contractStatus==Definitions.ContractStatus.DF', async () => {
    let result = await contractProxyContractDefaultConvention.testperformanceIndicator(3,{from: accounts[0]});
  });
  it('Should execute testperformanceIndicator(ContractStatus) WHEN contractStatus!=Definitions.ContractStatus.DF', async () => {
    let result = await contractProxyContractDefaultConvention.testperformanceIndicator(1,{from: accounts[0]});
  });
});
