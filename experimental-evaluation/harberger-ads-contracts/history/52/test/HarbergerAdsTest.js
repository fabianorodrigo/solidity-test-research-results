...
contract("HarbergerAds",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractHarbergerAds = await HarbergerAds.new(contractERC20test.address,29,5,{from:accounts[0]});
    if(trace) console.log('SUCESSO: HarbergerAds.new(contractERC20test.address,29,5,{from:accounts[0]}');
  });
  ...
  it('Should execute addProperty(uint256)', async () => {
    let result = await contractHarbergerAds.addProperty(3,{from: accounts[0]});
  });
  ...
  it('Should execute buy(uint256,uint256,uint256) WHEN seller!=msg.sender', async () => {
    let result = await contractHarbergerAds.buy(1532892062, 1331007098, 1532892064,{from: accounts[3]});
  });
  it('Should execute buy(uint256,uint256,uint256) WHEN seller==msg.sender', async () => {
    let result = await contractHarbergerAds.buy(29, 17, 29,{from: accounts[7]});
  });
  ...
  it('Should execute changeRecipient(uint256,address)', async () => {
    let result = await contractHarbergerAds.changeRecipient(65, accounts[6],{from: accounts[0]});
  });
});
