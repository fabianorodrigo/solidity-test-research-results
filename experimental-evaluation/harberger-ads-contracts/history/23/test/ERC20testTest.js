...
contract("ERC20test",(accounts)=>{
  ...
  it('Should execute burn(uint256) WHEN _value<=balances', async () => {
    let result = await contractERC20test.burn(2014223715,{from: accounts[0]});
  });
  it('Should execute mint(address) WHEN mintingFinished!=true', async () => {
    let result = await contractERC20test.methods["mint(address)"](accounts[7],{from: accounts[0]});
  });
  it('Should fail mint(address) when NOT comply with: mintingFinished != true', async () => {
    ...
    let result = await truffleAssert.fails(localERC20test.methods["mint(address)"](accounts[7],{from: accounts[0]}),'revert');
  });
});
