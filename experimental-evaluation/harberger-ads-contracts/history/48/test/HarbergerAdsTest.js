...
contract("HarbergerAds",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractHarbergerAds = await HarbergerAds.new(contractERC20test.address,28,4,{from:accounts[0]});
    if(trace) console.log('SUCESSO: HarbergerAds.new(contractERC20test.address,28,4,{from:accounts[0]}');
  });
  
  it('Should execute taxesDue(uint256) WHEN 1>0', async () => {
    let result = await contractHarbergerAds.taxesDue(26,{from: accounts[0]});
  });
  ...
  it('Should execute buy(uint256,uint256,uint256) WHEN seller!=msg.sender', async () => {
    let result = await contractHarbergerAds.buy(2014223715, 2014223714, 97,{from: accounts[9]});
  });
  it('Should execute buy(uint256,uint256,uint256) WHEN seller==msg.sender', async () => {
    let result = await contractHarbergerAds.buy(95, 19, 17,{from: accounts[5]});
  });
  ...
  it('Should execute changeRecipient(uint256,address)', async () => {
    let result = await contractHarbergerAds.changeRecipient(1532892063, accounts[6],{from: accounts[0]});
  });
});
