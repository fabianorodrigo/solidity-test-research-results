...
contract("HarbergerAds",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractHarbergerAds = await HarbergerAds.new(contractERC20test.address,3,27,{from:accounts[0]});
    if(trace) console.log('SUCESSO: HarbergerAds.new(contractERC20test.address,3,27,{from:accounts[0]}');
  });
  ...
  it('Should execute addProperty(uint256)', async () => {
    let result = await contractHarbergerAds.addProperty(2014223714,{from: accounts[0]});
  });
  ...
  it('Should execute buy(uint256,uint256,uint256) WHEN seller!=msg.sender', async () => {
    let result = await contractHarbergerAds.buy(5, 26, 1331007096,{from: accounts[1]});
  });
  it('Should execute buy(uint256,uint256,uint256) WHEN seller==msg.sender', async () => {
    let result = await contractHarbergerAds.buy(0, 1331007098, 5,{from: accounts[1]});
  });
  ...
  it('Should execute changeRecipient(uint256,address)', async () => {
    let result = await contractHarbergerAds.changeRecipient(66, accounts[6],{from: accounts[0]});
  });
});
