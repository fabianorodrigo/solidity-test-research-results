...
contract("HarbergerAds",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractHarbergerAds = await HarbergerAds.new(contractERC20test.address,1532892062,1331007098,{from:accounts[0]});
    if(trace) console.log('SUCESSO: HarbergerAds.new(contractERC20test.address,1532892062,1331007098,{from:accounts[0]}');
  });
  
  it('Should execute taxesDue(uint256) WHEN 1>0', async () => {
    let result = await contractHarbergerAds.taxesDue(65,{from: accounts[0]});
  });
  it('Should execute balanceOf(address) WHEN balance>allowance', async () => {
    let result = await contractHarbergerAds.balanceOf(accounts[8],{from: accounts[0]});
  });
  it('Should execute balanceOf(address) WHEN balance<=allowance', async () => {
    let result = await contractHarbergerAds.balanceOf(accounts[6],{from: accounts[0]});
  });
  it('Should execute addProperty(uint256)', async () => {
    let result = await contractHarbergerAds.addProperty(1331007096,{from: accounts[0]});
  });
  it('Should execute forecloseIfPossible(uint256)', async () => {
    let result = await contractHarbergerAds.forecloseIfPossible(1,{from: accounts[0]});
  });
  it('Should execute collectTaxes(uint256) WHEN taxes>0,taxes<=balance', async () => {
    let result = await contractHarbergerAds.collectTaxes(1532892064,{from: accounts[0]});
  });
  it('Should execute collectTaxes(uint256) WHEN taxes<=0,taxes<=balance', async () => {
    let result = await contractHarbergerAds.collectTaxes(1331007098,{from: accounts[0]});
  });
  it('Should execute collectTaxes(uint256) WHEN balance>0,taxes>balance', async () => {
    let result = await contractHarbergerAds.collectTaxes(66,{from: accounts[0]});
  });
  it('Should execute collectTaxes(uint256) WHEN balance<=0,taxes>balance', async () => {
    let result = await contractHarbergerAds.collectTaxes(28,{from: accounts[0]});
  });
  it('Should execute buy(uint256,uint256,uint256) WHEN seller!=msg.sender', async () => {
    let result = await contractHarbergerAds.buy(65, 0, 1,{from: accounts[6]});
  });
  it('Should execute buy(uint256,uint256,uint256) WHEN seller==msg.sender', async () => {
    let result = await contractHarbergerAds.buy(1331007097, 1331007098, 18,{from: accounts[9]});
  });
  ...
  it('Should execute changeRecipient(uint256,address)', async () => {
    let result = await contractHarbergerAds.changeRecipient(2014223716, accounts[6],{from: accounts[0]});
  });
});
