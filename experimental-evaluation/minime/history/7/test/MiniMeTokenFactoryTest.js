...
contract("MiniMeTokenFactory",(accounts)=>{
  ...
  it('Should execute createCloneToken(address,uint,string,uint8,string,bool)', async () => {
    let result = await contractMiniMeTokenFactory.createCloneToken(accounts[7], 10000, "\x19Ethereum Signed Message:\n32", 17, "ERC1820_ACCEPT_MAGIC", false,{from: accounts[0]});
  });
});
