...
contract("contractProxyVerifier",(accounts)=>{
  ...
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall!=true', async () => {
    let result = await contractProxyVerifier.testverify([57,59], {"A": {"X": 33,"Y": 25},"B": {"X": [24,3],"Y": [1532892064,11]},"C": {"X": 60,"Y": 16}},{from: accounts[0]});
  });
  it('Should execute testverify(uint[],Verifier.Proof) WHEN FunctionCall==true', async () => {
    let result = await contractProxyVerifier.testverify([26,88,257,255,15,200000,18,33,32,21], {"A": {"X": 257,"Y": 2014223715},"B": {"X": [98,29],"Y": [2014223715,22]},"C": {"X": 162,"Y": 18}},{from: accounts[0]});
  });
});
