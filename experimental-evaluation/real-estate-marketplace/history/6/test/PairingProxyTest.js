...
contract("contractProxyPairing",(accounts)=>{
  ...
  it('Should execute testnegate(Pairing.G1Point) WHEN p.X!=0', async () => {
    let result = await contractProxyPairing.testnegate({"X": 33,"Y": 256},{from: accounts[0]});
  });
  it('Should execute testaddition0(Pairing.G1Point,Pairing.G1Point)', async () => {
    let result = await contractProxyPairing.testaddition0({"X": 7,"Y": 33}, {"X": 102,"Y": 71},{from: accounts[0]});
  });
  it('Should execute testaddition1(Pairing.G2Point,Pairing.G2Point)', async () => {
    let result = await contractProxyPairing.testaddition1({"X": [95,127],"Y": [98,1023]}, {"X": [58,56],"Y": [1532892062,257]},{from: accounts[0]});
  });
  it('Should execute testscalar_mul(Pairing.G1Point,uint)', async () => {
    let result = await contractProxyPairing.testscalar_mul({"X": 86,"Y": 56}, 254,{from: accounts[0]});
  });
  it('Should execute testpairing(Pairing.G1Point[],Pairing.G2Point[]) WHEN p1.length==p2.length', async () => {
    let result = await contractProxyPairing.testpairing([{"X": 102,"Y": 4},{"X": 61,"Y": 98},{"X": 2014223715,"Y": 8}], [{"X": [200001,19],"Y": [69,28]},{"X": [88,20],"Y": [32,31]},{"X": [71,60],"Y": [102,71]}],{from: accounts[0]});
  });
  it('Should fail testpairing(Pairing.G1Point[],Pairing.G2Point[]) when NOT comply with: p1.length == p2.length', async () => {
    let result = await truffleAssert.fails(contractProxyPairing.testpairing([{"X": 6,"Y": 66},{"X": 4,"Y": 97},{"X": 47,"Y": 27},{"X": 162,"Y": 257}], [{"X": [200001,19],"Y": [69,28]},{"X": [88,20],"Y": [32,31]},{"X": [71,60],"Y": [102,71]}],{from: accounts[0]}),'revert');
  });
  it('Should execute testpairingProd2(Pairing.G1Point,Pairing.G2Point,Pairing.G1Point,Pairing.G2Point)', async () => {
    let result = await contractProxyPairing.testpairingProd2({"X": 2014223715,"Y": 48}, {"X": [18,0],"Y": [65,65]}, {"X": 46,"Y": 57}, {"X": [5,65],"Y": [63,127]},{from: accounts[0]});
  });
  it('Should execute testpairingProd3(Pairing.G1Point,Pairing.G2Point,Pairing.G1Point,Pairing.G2Point,Pairing.G1Point,Pairing.G2Point)', async () => {
    let result = await contractProxyPairing.testpairingProd3({"X": 2014223714,"Y": 45}, {"X": [45,2014223714],"Y": [27,2]}, {"X": 16,"Y": 5}, {"X": [15,103],"Y": [60,28]}, {"X": 2014223716,"Y": 2014223715}, {"X": [98,20],"Y": [97,8]},{from: accounts[0]});
  });
});
