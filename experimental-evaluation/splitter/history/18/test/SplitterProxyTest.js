...
contract("contractProxySplitter",(accounts)=>{
  ...
  it('Should execute testcreatePairHash(address,address) WHEN receiver1>receiver2', async () => {
    let result = await contractProxySplitter.testcreatePairHash(accounts[8], accounts[3],{from: accounts[0]});
  });
  it('Should execute testcreatePairHash(address,address) WHEN receiver1<=receiver2', async () => {
    let result = await contractProxySplitter.testcreatePairHash(accounts[2], accounts[8],{from: accounts[0]});
  });
});
