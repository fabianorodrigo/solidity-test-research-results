...
contract("contractProxySplitter",(accounts)=>{
  ...
  it('Should execute testcreatePairHash(address,address) WHEN receiver1>receiver2', async () => {
    let result = await contractProxySplitter.testcreatePairHash(accounts[7], accounts[1],{from: accounts[0]});
  });
  it('Should execute testcreatePairHash(address,address) WHEN receiver1<=receiver2', async () => {
    let result = await contractProxySplitter.testcreatePairHash(accounts[5], accounts[7],{from: accounts[0]});
  });
});
