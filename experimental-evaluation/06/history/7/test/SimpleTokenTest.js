...
contract("SimpleToken",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractSimpleCrowdfund = await SimpleCrowdfund.new(501,1,{from:accounts[0]});
    if(trace) console.log('SUCESSO: SimpleCrowdfund.new(501,1,{from:accounts[0]}');
  });
  ...
  it('Should execute transfer(address,uint256) WHEN _to!=0x0000000000000000000000000000000000000000,_value<=balances', async () => {
    let result = await contractSimpleToken.transfer(accounts[0], 1001,{from: accounts[0]});
  });
  ...
  it('Should execute mint(address,uint256) WHEN msg.sender==_owner,totalSupply<maxSupply', async () => {
    let result = await contractSimpleToken.mint(accounts[8], 7,{from: accounts[0]});
  });
  it('Should fail mint(address,uint256) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractSimpleToken.mint(accounts[8], 7,{from: accounts[9]}),'revert');
  });
