...
contract("SimpleToken",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractSimpleCrowdfund = await SimpleCrowdfund.new(500,8,{from:accounts[0]});
    if(trace) console.log('SUCESSO: SimpleCrowdfund.new(500,8,{from:accounts[0]}');
  });
  
  it('Should execute transferFrom(address,address,uint256)', async () => {
    let result = await contractSimpleToken.transferFrom(accounts[3], accounts[6], 19,{from: accounts[0]});
  });
  it('Should execute approve(address,uint256)', async () => {
    let result = await contractSimpleToken.approve(accounts[6], 750,{from: accounts[0]});
  });
  it('Should execute allowance(address,address)', async () => {
    let result = await contractSimpleToken.allowance(accounts[7], accounts[1],{from: accounts[0]});
  });
  ...
  it('Should execute mint(address,uint256) WHEN msg.sender==_owner,totalSupply<maxSupply', async () => {
    let result = await contractSimpleToken.mint(accounts[3], 17,{from: accounts[0]});
  });
  it('Should fail mint(address,uint256) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractSimpleToken.mint(accounts[3], 17,{from: accounts[9]}),'revert');
  });
