...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setRoundNum(uint256)', async () => {
    let result = await contractRelevantTokenMock.setRoundNum(1000000,{from: accounts[0]});
  });
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("\x19Ethereum Signed Message:\n32",103,"\x19Ethereum Signed Message:\n32","PayableExample",accounts[0],2014223716,1532892062,88,50,10001,115,38,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(129, 90, 41,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("\x19Ethereum Signed Message:\n32",103,"\x19Ethereum Signed Message:\n32","PayableExample",accounts[0],2014223716,1532892062,88,50,10001,115,38,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(129, 90, 41,{from: accounts[9]}),'revert');
  });
