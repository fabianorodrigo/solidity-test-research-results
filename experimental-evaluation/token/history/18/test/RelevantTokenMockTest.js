...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("RevertWithReason",83,"tbww2","UsesExample",accounts[0],94,38,47,75,982081,125,55,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(50, 27, 11,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("RevertWithReason",83,"tbww2","UsesExample",accounts[0],94,38,47,75,982081,125,55,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(50, 27, 11,{from: accounts[9]}),'revert');
  });
