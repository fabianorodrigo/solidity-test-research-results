...
contract("contractProxyPower",(accounts)=>{
  ...
  it('Should execute testpower(uint256,uint256,uint32,uint32)', async () => {
    let result = await contractProxyPower.testpower(92, 49, 75, 104,{from: accounts[0]});
  });
  it('Should execute testln(uint256,uint256) WHEN _numerator<=MAX_NUM', async () => {
    let result = await contractProxyPower.testln(50, 114,{from: accounts[0]});
  });
  it('Should fail testln(uint256,uint256) when NOT comply with: _numerator <= MAX_NUM', async () => {
    let result = await truffleAssert.fails(contractProxyPower.testln(new web3.utils.BN('6.80564733841877e+38'), 114,{from: accounts[0]}),'revert');
  });
  it('Should execute testfloorLog2(uint256)', async () => {
    let result = await contractProxyPower.testfloorLog2(76,{from: accounts[0]});
  });
  ...
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray<_x,maxExpArray<_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(256,{from: accounts[0]});
  });
  it('Should execute testfixedExp(uint256,uint8)', async () => {
    let result = await contractProxyPower.testfixedExp(129, 66,{from: accounts[0]});
  });
});
