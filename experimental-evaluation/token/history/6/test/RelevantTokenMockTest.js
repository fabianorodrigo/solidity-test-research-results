...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("Example",57,"ju20h","dik81",accounts[2],31,75,114,58,59,44,114,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(65, 35, 257,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("Example",57,"ju20h","dik81",accounts[2],31,75,114,58,59,44,114,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(65, 35, 257,{from: accounts[9]}),'revert');
  });
