...
contract("contractProxyRelevantToken",(accounts)=>{
  ...
  it('Should execute testnewTokensForConstantPhase(uint256,uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForConstantPhase(122, 99,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound==0', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(58,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound!=0', async () => {
    await contractProxyRelevantToken.releaseTokens({from:accounts[0]});
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(110,{from: accounts[0]});
  });
  ...
  it('Should execute testsplitRewards(uint256)', async () => {
    let result = await contractProxyRelevantToken.testsplitRewards(95,{from: accounts[0]});
  });
