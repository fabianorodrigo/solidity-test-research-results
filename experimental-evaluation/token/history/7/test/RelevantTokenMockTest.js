...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setRoundNum(uint256)', async () => {
    let result = await contractRelevantTokenMock.setRoundNum(106,{from: accounts[0]});
  });
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("IsLibrary",55,"0.3","Example",accounts[6],61,46,103,71,32737,73,115,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(108, 38, 98,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("IsLibrary",55,"0.3","Example",accounts[6],61,46,103,71,32737,73,115,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(108, 38, 98,{from: accounts[9]}),'revert');
  });
