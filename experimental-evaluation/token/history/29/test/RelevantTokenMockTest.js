...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setRoundNum(uint256)', async () => {
    let result = await contractRelevantTokenMock.setRoundNum(108,{from: accounts[0]});
  });
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("7h4zhl",98,"7h4zhl","PayableExample",accounts[1],126,77,1338,44,128,41,3,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(10, 59, 1532892064,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("7h4zhl",98,"7h4zhl","PayableExample",accounts[1],126,77,1338,44,128,41,3,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(10, 59, 1532892064,{from: accounts[9]}),'revert');
  });
