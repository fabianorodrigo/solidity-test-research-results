...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setRoundNum(uint256)', async () => {
    let result = await contractRelevantTokenMock.setRoundNum(10000,{from: accounts[0]});
  });
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("PayableExample",43,"\x19Ethereum Signed Message:\n32","27srf",accounts[5],10001,114,31,100,42,78,100,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(38, 60, 96,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("PayableExample",43,"\x19Ethereum Signed Message:\n32","27srf",accounts[5],10001,114,31,100,42,78,100,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(38, 60, 96,{from: accounts[9]}),'revert');
  });
