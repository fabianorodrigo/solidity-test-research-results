...
contract("TestERC20BondingToken",(accounts)=>{
  ...
  it('Should execute initialize(ERC20,uint256,uint256,uint32,uint256) WHEN initialized!=true', async () => {
    let result = await contractTestERC20BondingToken.methods["initialize(address,uint256,uint256,uint32,uint256)"](contractTestEthBondingToken.address, 2014223714, 47, 255, 88,{from: accounts[0]});
  });
  it('Should execute mint(uint256) WHEN msg.gasprice<=gasPrice,amount>0,msg.sender!=0', async () => {
    let result = await contractTestERC20BondingToken.mint(982081,{from: accounts[7]});
  });
  it('Should fail mint(uint256) when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractTestERC20BondingToken.mint(982081,{from: accounts[7]}),'revert');
  });
  it('Should fail mint(uint256) when NOT comply with: amount > 0', async () => {
    let result = await truffleAssert.fails(contractTestERC20BondingToken.mint(0,{from: accounts[7]}),'revert');
  });
  it('Should fail mint(uint256) when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractTestERC20BondingToken.mint(982081,{from: 0}),'revert');
  });
  it('Should execute burn(uint256)', async () => {
    let result = await contractTestERC20BondingToken.burn(92,{from: accounts[0]});
  });
});
