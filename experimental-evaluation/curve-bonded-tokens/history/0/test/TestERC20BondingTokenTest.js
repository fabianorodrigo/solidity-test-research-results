...
contract("TestERC20BondingToken",(accounts)=>{
  ...
  it('Should execute initialize(ERC20,uint256,uint256,uint32,uint256) WHEN initialized!=true', async () => {
    let result = await contractTestERC20BondingToken.methods["initialize(address,uint256,uint256,uint32,uint256)"](contractEthBondingToken.address, 1532892062, 0, 41, 58,{from: accounts[0]});
  });
  it('Should execute mint(uint256) WHEN msg.gasprice<=gasPrice,amount>0,msg.sender!=0', async () => {
    let result = await contractTestERC20BondingToken.mint(49,{from: accounts[6]});
  });
  it('Should fail mint(uint256) when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractTestERC20BondingToken.mint(49,{from: accounts[6]}),'revert');
  });
  ...
  it('Should fail mint(uint256) when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractTestERC20BondingToken.mint(49,{from: 0}),'revert');
  });
  it('Should execute burn(uint256)', async () => {
    let result = await contractTestERC20BondingToken.burn(255,{from: accounts[0]});
  });
});
