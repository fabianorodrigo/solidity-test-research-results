...
contract("TestEthBondingToken",(accounts)=>{
  ...
  it('Should execute initialize(uint256,uint32,uint256) WHEN initialized!=true', async () => {
    let result = await contractTestEthBondingToken.initialize(79, 982080, 1,{from: accounts[0]});
  });
  it('Should execute fallback() WHEN msg.value>0,msg.gasprice<=gasPrice,msg.value>0,msg.sender!=0', async () => {
    let result = await contractTestEthBondingToken.sendTransaction({from: accounts[5],value:81});
  });
  ...
  it('Should fail fallback() when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.sendTransaction({from: accounts[5],value:81}),'revert');
  });
  it('Should fail fallback() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.sendTransaction({from: accounts[5]}),'revert');
  });
  it('Should fail fallback() when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.sendTransaction({from: 0,value:81}),'revert');
  });
  it('Should execute mint() WHEN msg.value>0,msg.gasprice<=gasPrice,msg.value>0,msg.sender!=0', async () => {
    let result = await contractTestEthBondingToken.mint({from: accounts[0],value:34});
  });
  ...
  it('Should fail mint() when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.mint({from: accounts[0],value:34}),'revert');
  });
  it('Should fail mint() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.mint({from: accounts[0]}),'revert');
  });
  it('Should fail mint() when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.mint({from: 0,value:34}),'revert');
  });
  it('Should execute burn(uint256)', async () => {
    let result = await contractTestEthBondingToken.burn(124,{from: accounts[0]});
  });
});
