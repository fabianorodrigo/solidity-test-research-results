...
contract("contractProxyEthBondingToken",(accounts)=>{
  ...
  it('Should execute test_curvedMint() WHEN msg.value>0,msg.gasprice<=gasPrice,msg.value>0,msg.sender!=0', async () => {
    let result = await contractProxyEthBondingToken.test_curvedMint({from: accounts[7],value:101});
  });
  it('Should fail test_curvedMint() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(contractProxyEthBondingToken.test_curvedMint({from: accounts[7]}),'revert');
  });
  it('Should fail test_curvedMint() when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractProxyEthBondingToken.test_curvedMint({from: accounts[7],value:101}),'revert');
  });
  it('Should fail test_curvedMint() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(contractProxyEthBondingToken.test_curvedMint({from: accounts[7]}),'revert');
  });
  it('Should fail test_curvedMint() when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractProxyEthBondingToken.test_curvedMint({from: 0,value:101}),'revert');
  });
  it('Should execute test_curvedBurn(uint256)', async () => {
    let result = await contractProxyEthBondingToken.test_curvedBurn(98,{from: accounts[0]});
  });
});
