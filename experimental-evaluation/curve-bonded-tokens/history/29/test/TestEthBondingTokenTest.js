...
contract("TestEthBondingToken",(accounts)=>{
  ...
  it('Should execute initialize(uint256,uint32,uint256) WHEN initialized!=true', async () => {
    let result = await contractTestEthBondingToken.initialize(55, 257, 71,{from: accounts[0]});
  });
  it('Should execute fallback() WHEN msg.value>0,msg.gasprice<=gasPrice,msg.value>0,msg.sender!=0', async () => {
    let result = await contractTestEthBondingToken.sendTransaction({from: accounts[8],value:257});
  });
  it('Should fail fallback() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.sendTransaction({from: accounts[8]}),'revert');
  });
  it('Should fail fallback() when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.sendTransaction({from: accounts[8],value:257}),'revert');
  });
  it('Should fail fallback() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.sendTransaction({from: accounts[8]}),'revert');
  });
  it('Should fail fallback() when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.sendTransaction({from: 0,value:257}),'revert');
  });
  it('Should execute mint() WHEN msg.value>0,msg.gasprice<=gasPrice,msg.value>0,msg.sender!=0', async () => {
    let result = await contractTestEthBondingToken.mint({from: accounts[7],value:27});
  });
  ...
  it('Should fail mint() when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.mint({from: accounts[7],value:27}),'revert');
  });
  it('Should fail mint() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.mint({from: accounts[7]}),'revert');
  });
  it('Should fail mint() when NOT comply with: msg.sender != 0', async () => {
    let result = await truffleAssert.fails(contractTestEthBondingToken.mint({from: 0,value:27}),'revert');
  });
  it('Should execute burn(uint256)', async () => {
    let result = await contractTestEthBondingToken.burn(55,{from: accounts[0]});
  });
});
