...
contract("Aiakos",(accounts)=>{
  ...
  it('Should execute deployRelease(string,bytes32) WHEN MemberAccess.Identifier!=true', async () => {
    let result = await contractAiakos.deployRelease("bouncer", [181,211,184,76,134,121,15,153,122,208,35,131,224,52,208,2,50,10,195,103,80,123,30,33,90,253,179,62,223,176,91,195],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN MemberAccess.Identifier==true', async () => {
    let result = await contractAiakos.deployRelease("PayableExample", [89,227,226,144,130,225,82,0,146,23,79,244,209,13,70,90,78,5,105,253,101,174,75,130,219,43,3,138,248,104,132,167],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN approvalGranted==true', async () => {
    let result = await contractAiakos.deployRelease("initialized", [240,0,50,160,3,230,36,2,163,184,175,252,45,210,238,129,100,149,94,175,115,156,85,69,254,35,186,161,224,188,110,172],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN approvalGranted!=true', async () => {
    let result = await contractAiakos.deployRelease("Example", [208,59,188,236,2,170,23,189,141,99,165,108,89,17,121,47,104,229,6,24,169,119,62,188,9,162,251,216,135,12,7,11],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN releaseApproved==true', async () => {
    let result = await contractAiakos.deployRelease("IsLibrary", [37,202,24,102,145,77,0,245,96,99,73,221,139,41,37,208,209,161,2,174,134,87,76,203,67,245,251,48,8,99,191,211],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN releaseApproved!=true,approvalGranted!=true', async () => {
    let result = await contractAiakos.deployRelease("ERC1820_ACCEPT_MAGIC", [141,224,93,40,249,64,224,152,131,167,14,162,181,34,178,143,220,164,134,136,146,92,101,68,254,19,48,73,25,113,82,106],{from: accounts[0]});
  });
  it('Should execute checkRelease(string,bytes32) WHEN _sha256Hash!=0x0,MemberAccess.Identifier==true', async () => {
    let result = await contractAiakos.checkRelease("\x19Ethereum Signed Message:\n32", [204,126,195,86,57,11,135,13,97,147,130,240,246,65,66,16,113,140,68,119,228,192,30,213,229,49,8,106,85,162,203,140],{from: accounts[0]});
  });
  ...
  it('Should execute getReleaseInfo(string)', async () => {
    let result = await contractAiakos.getReleaseInfo("\x19Ethereum Signed Message:\n32",{from: accounts[0]});
  });
  ...
  it('Should execute transferOwnership(address) WHEN _newOwner!=0x0000000000000000000000000000000000000000,msg.sender==_owner', async () => {
    let result = await contractAiakos.methods["transferOwnership(address)"](accounts[6],{from: accounts[0]});
  });
  ...
  it('Should fail transferOwnership(address) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractAiakos.methods["transferOwnership(address)"](accounts[6],{from: accounts[9]}),'revert');
  });
});
