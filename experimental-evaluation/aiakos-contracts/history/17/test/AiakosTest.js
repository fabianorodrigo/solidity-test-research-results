...
contract("Aiakos",(accounts)=>{
  ...
  it('Should execute deployRelease(string,bytes32) WHEN MemberAccess.Identifier!=true', async () => {
    let result = await contractAiakos.deployRelease("initialized", [4,227,33,126,200,5,107,155,111,240,133,90,180,140,26,131,150,128,106,10,253,54,147,18,128,70,10,66,240,113,21,17],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN MemberAccess.Identifier==true', async () => {
    let result = await contractAiakos.deployRelease("superuser", [105,137,111,158,34,91,240,79,227,203,127,50,101,4,79,72,194,206,162,254,97,140,27,131,35,159,53,252,78,145,26,50],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN approvalGranted==true', async () => {
    let result = await contractAiakos.deployRelease("initialized", [168,154,117,147,192,109,149,203,18,79,178,111,167,127,237,103,57,5,231,67,53,183,146,214,138,109,101,84,156,74,246,132],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN approvalGranted!=true', async () => {
    let result = await contractAiakos.deployRelease("Example", [201,227,96,169,133,113,143,111,153,159,1,121,121,115,184,169,124,153,31,199,24,102,231,129,4,14,251,178,4,184,208,60],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN releaseApproved==true', async () => {
    let result = await contractAiakos.deployRelease("ERC1820_ACCEPT_MAGIC", [149,219,83,105,208,66,31,140,80,91,123,26,201,120,104,51,180,32,121,248,130,54,137,152,140,201,2,167,178,74,159,188],{from: accounts[0]});
  });
  it('Should execute deployRelease(string,bytes32) WHEN releaseApproved!=true,approvalGranted!=true', async () => {
    let result = await contractAiakos.deployRelease("superuser", [72,236,185,45,8,155,83,122,150,21,232,199,211,250,162,55,42,65,203,92,38,54,141,102,242,228,68,213,45,33,14,218],{from: accounts[0]});
  });
  it('Should execute checkRelease(string,bytes32) WHEN _sha256Hash!=0x0,MemberAccess.Identifier==true', async () => {
    let result = await contractAiakos.checkRelease("RevertWithReason", [164,168,80,57,140,89,110,126,250,130,49,195,189,20,192,101,210,73,151,196,231,100,54,193,184,214,232,96,232,17,229,106],{from: accounts[0]});
  });
