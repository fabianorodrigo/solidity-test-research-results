...
contract("SimpleCoin",(accounts)=>{
  ...
  it('Should execute balanceOf(address)', async () => {
    let result = await contractSimpleCoin.balanceOf(accounts[4],{from: accounts[0]});
  });
  ...
  it('Should execute transfer(address,uint256) WHEN balances>=_value', async () => {
    let result = await contractSimpleCoin.transfer(accounts[3], 0,{from: accounts[0]});
  });
  it('Should execute transferFrom(address,address,uint256) WHEN balances>=_value,remaining>=_value', async () => {
    let result = await contractSimpleCoin.transferFrom(accounts[2], accounts[8], 999999,{from: accounts[0]});
  });
  it('Should execute approve(address,uint256)', async () => {
    let result = await contractSimpleCoin.approve(accounts[7], 0,{from: accounts[0]});
  });
