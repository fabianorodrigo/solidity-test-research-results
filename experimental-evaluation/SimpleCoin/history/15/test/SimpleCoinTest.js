...
contract("SimpleCoin",(accounts)=>{
  ...
  it('Should execute allowance(address,address)', async () => {
    let result = await contractSimpleCoin.allowance(accounts[2], accounts[1],{from: accounts[0]});
  });
  ...
  it('Should execute transfer(address,uint256) WHEN balances>=_value', async () => {
    let result = await contractSimpleCoin.transfer(accounts[0], 1000000,{from: accounts[0]});
  });
  it('Should execute transferFrom(address,address,uint256) WHEN balances>=_value,remaining>=_value', async () => {
    let result = await contractSimpleCoin.transferFrom(accounts[8], accounts[1], 0,{from: accounts[0]});
  });
