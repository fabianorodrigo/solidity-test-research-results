...
contract("SimpleCoin",(accounts)=>{
  ...
  it('Should execute balanceOf(address)', async () => {
    let result = await contractSimpleCoin.balanceOf(accounts[7],{from: accounts[0]});
  });
  it('Should execute allowance(address,address)', async () => {
    let result = await contractSimpleCoin.allowance(accounts[3], accounts[0],{from: accounts[0]});
  });
  ...
  it('Should execute transferFrom(address,address,uint256) WHEN balances>=_value,remaining>=_value', async () => {
    let result = await contractSimpleCoin.transferFrom(accounts[1], accounts[5], 1000000,{from: accounts[0]});
  });
  it('Should execute approve(address,uint256)', async () => {
    let result = await contractSimpleCoin.approve(accounts[6], 1000001,{from: accounts[0]});
  });
  ...
  it('Should execute acceptOwnership() WHEN msg.sender==newOwner', async () => {
    ...
    let result = await contractSimpleCoin.acceptOwnership({from: accounts[5]});
  });
