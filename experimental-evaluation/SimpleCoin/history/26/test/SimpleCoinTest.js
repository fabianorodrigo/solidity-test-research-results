...
contract("SimpleCoin",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN balances>=_value', async () => {
    let result = await contractSimpleCoin.transfer(accounts[0], 999999,{from: accounts[0]});
  });
  it('Should execute transferFrom(address,address,uint256) WHEN balances>=_value,remaining>=_value', async () => {
    let result = await contractSimpleCoin.transferFrom(accounts[7], accounts[6], 999999,{from: accounts[0]});
  });
  it('Should execute approve(address,uint256)', async () => {
    let result = await contractSimpleCoin.approve(accounts[7], 1000000,{from: accounts[0]});
  });
