...
contract("EIP20",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN balances>=_value', async () => {
    let result = await contractEIP20.transfer(accounts[1], 101,{from: accounts[0]});
  });
  it('Should execute transferFrom(address,address,uint256) WHEN allowance<MAX_UINT256,balances>=_value,allowance>=_value', async () => {
    let result = await contractEIP20.transferFrom(accounts[0], accounts[4], 100,{from: accounts[0]});
  });
  it('Should execute transferFrom(address,address,uint256) WHEN allowance>=MAX_UINT256,balances>=_value,allowance>=_value', async () => {
    let result = await contractEIP20.transferFrom(accounts[4], accounts[1], 100,{from: accounts[0]});
  });
