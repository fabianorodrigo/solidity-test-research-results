...
contract("contractProxySafeMath",(accounts)=>{
  ...
  it('Should fail testmul(uint256,uint256) when NOT comply with: a == 0', async () => {
    let result = await truffleAssert.fails(contractProxySafeMath.testmul(1, 99,{from: accounts[0]}),'revert');
  });
  it('Should execute testdiv(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testdiv(100, 101,{from: accounts[0]});
  });
  it('Should execute testsub(uint256,uint256) WHEN b<=a', async () => {
    let result = await contractProxySafeMath.testsub(9999, 9999,{from: accounts[0]});
  });
  ...
  it('Should execute testadd(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testadd(100, 100,{from: accounts[0]});
  });
});
