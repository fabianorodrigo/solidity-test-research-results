...
contract("contractProxyBaseSwaps",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractBaseSwaps = await BaseSwaps.new(accounts[0],accounts[3],accounts[8],3,4038714811,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+972,{from:accounts[0]});
    if(trace) console.log('SUCESSO: BaseSwaps.new(accounts[0],accounts[3],accounts[8],3,4038714811,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+972,{from:accounts[0]}');
      ProxyBaseSwaps.link("SafeMath",contractSafeMath.address);
    contractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[0],accounts[3],accounts[8],3,4038714811,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+972,{from:accounts[0]});
});
  ...
  it('Should execute test_sendTokens(address,address,uint) WHEN _token==0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens("0x0000000000000000000000000000000000000000", accounts[4], 254,{from: accounts[0]});
  });
  it('Should execute test_sendTokens(address,address,uint) WHEN _token!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens(accounts[0], accounts[2], 255,{from: accounts[0]});
  });
  it('Should execute test_removeInvestor(address[],address)', async () => {
    let result = await contractProxyBaseSwaps.test_removeInvestor([accounts[2],accounts[9],accounts[2],accounts[3],accounts[0],accounts[1],accounts[6],accounts[2],accounts[6]], accounts[5],{from: accounts[0]});
  });
  ...
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall!=true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[3], accounts[7], 2014223715,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall==true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[6], accounts[4], 6,{from: accounts[0]});
  });
