...
contract("BaseSwaps",(accounts)=>{
  ...
  it('Should execute fallback() WHEN quoteAddress==0x0000000000000000000000000000000000000000,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let localBaseSwaps = await BaseSwaps.new(accounts[3],accounts[9],"0x0000000000000000000000000000000000000000",66,1,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+539,{from:accounts[0]});
  ...
  it('Should fail fallback() when NOT comply with: quoteAddress == 0x0000000000000000000000000000000000000000', async () => {
    let localBaseSwaps = await BaseSwaps.new(accounts[3],accounts[4],accounts[6],9999,9999,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+980,{from:accounts[0]});
  ...
  it('Should execute depositBaseTokens(uint) WHEN _value<=allowance,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractBaseSwaps.depositBaseTokens(1532892064,{from: accounts[0]});
  });
  ...
  it('Should fail cancel() when NOT comply with: isSwapped != true', async () => {
    let localBaseSwaps = await BaseSwaps.new(accounts[6],"0x0000000000000000000000000000000000000000",accounts[3],64,1532892062,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+354,{from:accounts[0]});
  ...
  it('Should fail refundBase() when NOT comply with: isSwapped != true', async () => {
    let localBaseSwaps = await BaseSwaps.new(accounts[8],accounts[9],"0x0000000000000000000000000000000000000000",10001,2014223714,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+259,{from:accounts[0]});
    await localBaseSwaps.sendTransaction({from:accounts[0]});
  ...
  it('Should fail refundQuote() when NOT comply with: isSwapped != true', async () => {
    let localBaseSwaps = await BaseSwaps.new(accounts[0],accounts[1],"0x0000000000000000000000000000000000000000",4038714810,95,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+955,{from:accounts[0]});
    await localBaseSwaps.sendTransaction({from:accounts[0]});
