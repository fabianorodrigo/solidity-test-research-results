...
contract("contractProxyBaseSwaps",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractBaseSwaps = await BaseSwaps.new(accounts[3],accounts[6],accounts[5],256,66,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+464,{from:accounts[0]});
    if(trace) console.log('SUCESSO: BaseSwaps.new(accounts[3],accounts[6],accounts[5],256,66,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+464,{from:accounts[0]}');
      ProxyBaseSwaps.link("SafeMath",contractSafeMath.address);
    contractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[3],accounts[6],accounts[5],256,66,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+464,{from:accounts[0]});
});
  
  it('Should execute test_refund(address) WHEN isSwapped!=true', async () => {
    let result = await contractProxyBaseSwaps.test_refund(accounts[0],{from: accounts[0]});
  });
  it('Should fail test_refund(address) when NOT comply with: isSwapped != true', async () => {
    ...
    let result = await truffleAssert.fails(contractProxyBaseSwaps.test_refund(accounts[0],{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute test_distribute(address,address)', async () => {
    let result = await contractProxyBaseSwaps.test_distribute(accounts[5], accounts[1],{from: accounts[0]});
  });
  it('Should execute test_sendTokens(address,address,uint) WHEN _token==0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens("0x0000000000000000000000000000000000000000", accounts[1], 1532892062,{from: accounts[0]});
  });
  it('Should execute test_sendTokens(address,address,uint) WHEN _token!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens(accounts[0], accounts[5], 10001,{from: accounts[0]});
  });
  it('Should execute test_removeInvestor(address[],address)', async () => {
    let result = await contractProxyBaseSwaps.test_removeInvestor([accounts[5],accounts[6],accounts[6],accounts[2],accounts[7],accounts[1],accounts[0],accounts[9],accounts[7],accounts[2]], accounts[4],{from: accounts[0]});
  });
  ...
  it('Should execute test_depositTokens(address,uint) WHEN baseAddress==_token,quoteAddress==_token,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_depositTokens(accounts[2], 256,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall!=true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[1], accounts[5], 96,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall==true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[3], accounts[7], 257,{from: accounts[0]});
  });
  it('Should execute test_isInvestor(address,address)', async () => {
    let result = await contractProxyBaseSwaps.test_isInvestor(accounts[3], accounts[1],{from: accounts[0]});
  });
});
