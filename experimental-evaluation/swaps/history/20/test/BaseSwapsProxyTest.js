...
contract("contractProxyBaseSwaps",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractBaseSwaps = await BaseSwaps.new(accounts[0],accounts[0],accounts[8],65,254,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+153,{from:accounts[0]});
    if(trace) console.log('SUCESSO: BaseSwaps.new(accounts[0],accounts[0],accounts[8],65,254,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+153,{from:accounts[0]}');
      ProxyBaseSwaps.link("SafeMath",contractSafeMath.address);
    contractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[0],accounts[0],accounts[8],65,254,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+153,{from:accounts[0]});
});
  ...
  it('Should execute test_distribute(address,address)', async () => {
    let result = await contractProxyBaseSwaps.test_distribute(accounts[9], accounts[6],{from: accounts[0]});
  });
  it('Should execute test_sendTokens(address,address,uint) WHEN _token==0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens("0x0000000000000000000000000000000000000000", accounts[4], 1532892063,{from: accounts[0]});
  });
  it('Should execute test_sendTokens(address,address,uint) WHEN _token!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens(accounts[9], accounts[3], 4038714809,{from: accounts[0]});
  });
  it('Should execute test_removeInvestor(address[],address)', async () => {
    let result = await contractProxyBaseSwaps.test_removeInvestor([accounts[4],accounts[5],accounts[6]], accounts[6],{from: accounts[0]});
  });
  ...
  it('Should execute test_depositTokens(address,uint) WHEN baseAddress==_token,quoteAddress==_token,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_depositTokens(accounts[2], 10000,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall!=true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[1], accounts[9], 4038714811,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall==true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[9], accounts[4], 2,{from: accounts[0]});
  });
