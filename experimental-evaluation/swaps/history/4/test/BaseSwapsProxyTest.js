...
contract("contractProxyBaseSwaps",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractBaseSwaps = await BaseSwaps.new(accounts[6],accounts[7],accounts[3],9,4038714811,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+658,{from:accounts[0]});
    if(trace) console.log('SUCESSO: BaseSwaps.new(accounts[6],accounts[7],accounts[3],9,4038714811,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+658,{from:accounts[0]}');
      ProxyBaseSwaps.link("SafeMath",contractSafeMath.address);
    contractProxyBaseSwaps = await ProxyBaseSwaps.new(accounts[6],accounts[7],accounts[3],9,4038714811,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+658,{from:accounts[0]});
});
  ...
  it('Should fail test_refund(address) when NOT comply with: isSwapped != true', async () => {
    ...
    let result = await truffleAssert.fails(contractProxyBaseSwaps.test_refund(accounts[8],{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute test_sendTokens(address,address,uint) WHEN _token==0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens("0x0000000000000000000000000000000000000000", accounts[8], 11,{from: accounts[0]});
  });
  it('Should execute test_sendTokens(address,address,uint) WHEN _token!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyBaseSwaps.test_sendTokens(accounts[9], accounts[3], 9999,{from: accounts[0]});
  });
  it('Should execute test_removeInvestor(address[],address)', async () => {
    let result = await contractProxyBaseSwaps.test_removeInvestor([accounts[9],accounts[2],accounts[6],accounts[9],accounts[6],accounts[6]], accounts[0],{from: accounts[0]});
  });
  ...
  it('Should execute test_depositTokens(address,uint) WHEN baseAddress==_token,quoteAddress==_token,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_depositTokens(accounts[4], 28,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall!=true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[2], accounts[6], 1,{from: accounts[0]});
  });
  it('Should execute test_deposit(address,address,uint) WHEN FunctionCall==true,baseAddress==_token,quoteAddress==_token,raised<limits,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp<=expirationTimestamp', async () => {
    let result = await contractProxyBaseSwaps.test_deposit(accounts[2], accounts[8], 1532892064,{from: accounts[0]});
  });
  it('Should execute test_isInvestor(address,address)', async () => {
    let result = await contractProxyBaseSwaps.test_isInvestor(accounts[9], accounts[6],{from: accounts[0]});
  });
});
