...
contract("CustomOwnable",(accounts)=>{
  ...
  it('Should execute assignTrustee(address) WHEN msg.sender==_owner,account!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractCustomOwnable.assignTrustee(accounts[5],{from: accounts[0]});
  });
  it('Should fail assignTrustee(address) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractCustomOwnable.assignTrustee(accounts[5],{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute reassignOwner(address) WHEN msg.sender==_trustee,newOwner!=0x0000000000000000000000000000000000000000', async () => {
    ...
    let result = await contractCustomOwnable.reassignOwner(accounts[2],{from: accounts[6]});
  });
  it('Should fail reassignOwner(address) when NOT comply with: msg.sender == _trustee', async () => {
    await contractCustomOwnable.assignTrustee(accounts[6],{from:accounts[0]});
    let result = await truffleAssert.fails(contractCustomOwnable.reassignOwner(accounts[2],{from: accounts[9]}),'revert');
  });
