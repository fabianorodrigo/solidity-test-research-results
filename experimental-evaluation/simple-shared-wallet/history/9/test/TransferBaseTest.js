...
contract("TransferBase",(accounts)=>{
  ...
  it('Should execute transferTokens(address,address,uint256) WHEN _paused!=true', async () => {
    let result = await contractTransferBase.transferTokens(contractSimpleToken.address, accounts[4], 1532892064,{from: accounts[0]});
  });
  it('Should fail transferTokens(address,address,uint256) when NOT comply with: _paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractTransferBase.transferTokens(contractSimpleToken.address, accounts[4], 1532892064,{from: accounts[0]}),'revert');
  });
