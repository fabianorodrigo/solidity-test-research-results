...
contract("CappedTransfer",(accounts)=>{
  ...
  it('Should execute checkIfValidTransfer(uint256) WHEN _maximumTransfer>0,amount>0,amount<=_maximumTransfer', async () => {
    await contractCappedTransfer.setCap(28,1532892064,{from:accounts[0]});
    let result = await contractCappedTransfer.checkIfValidTransfer(28,{from: accounts[0]});
  });
  ...
  it('Should execute checkIfValidWeiTransfer(uint256) WHEN _maximumTransferWei>0,amount>0,amount<=_maximumTransferWei', async () => {
    await contractCappedTransfer.setCap(1532892064,2014223714,{from:accounts[0]});
    let result = await contractCappedTransfer.checkIfValidWeiTransfer(254,{from: accounts[0]});
  });
  ...
  it('Should fail checkIfValidWeiTransfer(uint256) when NOT comply with: amount <= _maximumTransferWei', async () => {
    await contractCappedTransfer.setCap(1532892064,2014223714,{from:accounts[0]});
    let result = await truffleAssert.fails(contractCappedTransfer.checkIfValidWeiTransfer(2014223715,{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute setCap(uint256,uint256) WHEN msg.sender==_owner,_paused!=true', async () => {
    let result = await contractCappedTransfer.setCap(5, 28,{from: accounts[0]});
  });
  it('Should fail setCap(uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractCappedTransfer.setCap(5, 28,{from: accounts[9]}),'revert');
  });
  it('Should fail setCap(uint256,uint256) when NOT comply with: _paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractCappedTransfer.setCap(5, 28,{from: accounts[0]}),'revert');
  });
