...
contract("BulkTransfer",(accounts)=>{
  ...
  it('Should execute bulkTransfer(address,address[],uint256[]) WHEN _paused!=true,destinations.length==amounts.length', async () => {
    let result = await contractBulkTransfer.bulkTransfer(contractSimpleToken.address, [accounts[6]], [255],{from: accounts[0]});
  });
  it('Should fail bulkTransfer(address,address[],uint256[]) when NOT comply with: _paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransfer(contractSimpleToken.address, [accounts[6]], [255],{from: accounts[0]}),'revert');
  });
  it('Should fail bulkTransfer(address,address[],uint256[]) when NOT comply with: destinations.length == amounts.length', async () => {
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransfer(contractSimpleToken.address, [accounts[6],accounts[5]], [255],{from: accounts[0]}),'revert');
  });
  it('Should execute bulkTransferEther(address[],uint256[]) WHEN _paused!=true,destinations.length==amounts.length', async () => {
    let result = await contractBulkTransfer.bulkTransferEther([accounts[8],accounts[5]], [255,28],{from: accounts[0]});
  });
  it('Should fail bulkTransferEther(address[],uint256[]) when NOT comply with: _paused != true', async () => {
    await contractBulkTransfer.pause({from:accounts[0]});
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransferEther([accounts[8],accounts[5]], [255,28],{from: accounts[0]}),'revert');
  });
  it('Should fail bulkTransferEther(address[],uint256[]) when NOT comply with: destinations.length == amounts.length', async () => {
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransferEther([accounts[1],accounts[9],accounts[6]], [255,28],{from: accounts[0]}),'revert');
  });
});
