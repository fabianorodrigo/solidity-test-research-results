...
contract("TransferBase",(accounts)=>{
  ...
  it('Should execute transferTokens(address,address,uint256) WHEN _paused!=true', async () => {
    let result = await contractTransferBase.transferTokens(contractSimpleToken.address, accounts[4], 1532892063,{from: accounts[0]});
  });
  it('Should fail transferTokens(address,address,uint256) when NOT comply with: _paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractTransferBase.transferTokens(contractSimpleToken.address, accounts[4], 1532892063,{from: accounts[0]}),'revert');
  });
  it('Should execute transferEthers(address,uint256) WHEN _paused!=true', async () => {
    let result = await contractTransferBase.transferEthers(accounts[5], 255,{from: accounts[0]});
  });
  it('Should fail transferEthers(address,uint256) when NOT comply with: _paused != true', async () => {
    await contractTransferBase.pause({from:accounts[0]});
    let result = await truffleAssert.fails(contractTransferBase.transferEthers(accounts[5], 255,{from: accounts[0]}),'revert');
  });
