...
contract("TransferBase",(accounts)=>{
  ...
  it('Should execute transferTokens(address,address,uint256) WHEN _paused!=true', async () => {
    let result = await contractTransferBase.transferTokens(contractSimpleToken.address, accounts[9], 65,{from: accounts[0]});
  });
  it('Should fail transferTokens(address,address,uint256) when NOT comply with: _paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractTransferBase.transferTokens(contractSimpleToken.address, accounts[9], 65,{from: accounts[0]}),'revert');
  });
  it('Should execute transferEthers(address,uint256) WHEN _paused!=true', async () => {
    let result = await contractTransferBase.transferEthers(accounts[6], 2014223716,{from: accounts[0]});
  });
  it('Should fail transferEthers(address,uint256) when NOT comply with: _paused != true', async () => {
    await contractTransferBase.pause({from:accounts[0]});
    let result = await truffleAssert.fails(contractTransferBase.transferEthers(accounts[6], 2014223716,{from: accounts[0]}),'revert');
  });
  it('Should execute tokenBalanceOf(address)', async () => {
    let result = await contractTransferBase.tokenBalanceOf(contractSimpleToken.address,{from: accounts[0]});
  });
  it('Should execute fallback() WHEN _paused!=true', async () => {
    let result = await contractTransferBase.sendTransaction({from: accounts[0]});
  });
  it('Should fail fallback() when NOT comply with: _paused != true', async () => {
    await contractTransferBase.pause({from:accounts[0]});
    let result = await truffleAssert.fails(contractTransferBase.sendTransaction({from: accounts[0]}),'revert');
  });
});
