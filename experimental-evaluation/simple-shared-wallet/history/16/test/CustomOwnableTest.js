...
contract("CustomOwnable",(accounts)=>{
  ...
  it('Should execute reassignOwner(address) WHEN msg.sender==_trustee,newOwner!=0x0000000000000000000000000000000000000000', async () => {
    ...
    let result = await contractCustomOwnable.reassignOwner(accounts[0],{from: accounts[9]});
  });
  it('Should fail reassignOwner(address) when NOT comply with: msg.sender == _trustee', async () => {
    await contractCustomOwnable.assignTrustee(accounts[9],{from:accounts[0]});
    let result = await truffleAssert.fails(contractCustomOwnable.reassignOwner(accounts[0],{from: accounts[8]}),'revert');
  });
