...
contract("LeftGalleryToken",(accounts)=>{
  ...
  it('Should execute tokenURI(uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenURI(1532892063,{from: accounts[0]});
  });
  it('Should execute tokenOfOwnerByIndex(address,uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenOfOwnerByIndex(accounts[3], 5,{from: accounts[0]});
  });
  ...
  it('Should execute tokenByIndex(uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenByIndex(32,{from: accounts[0]});
  });
  it('Should execute createWork(address,string) WHEN msg.sender==owner,_to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractLeftGalleryToken.createWork(accounts[0], "whitelist",{from: accounts[0]});
  });
  it('Should fail createWork(address,string) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractLeftGalleryToken.createWork(accounts[0], "whitelist",{from: accounts[9]}),'revert');
  });
