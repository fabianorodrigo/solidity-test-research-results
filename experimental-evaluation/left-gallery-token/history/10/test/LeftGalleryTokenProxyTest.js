...
contract("contractProxyLeftGalleryToken",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractERC721Token = await ERC721Token.new("lgbcs","superuser",{from:accounts[0]});
    if(trace) console.log('SUCESSO: ERC721Token.new("lgbcs","superuser",{from:accounts[0]}');
    ...
    contractToken = await Token.new("lgmm","lgbmocr",100,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Token.new("lgmm","lgbmocr",100,{from:accounts[0]}');
  ...
  it('Should execute test_setTokenURI(uint256,string)', async () => {
    let result = await contractProxyLeftGalleryToken.test_setTokenURI(2014223714, "lgmm",{from: accounts[0]});
  });
  it('Should execute testaddTokenTo(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testaddTokenTo(accounts[2], 1532892063,{from: accounts[0]});
  });
  it('Should execute testremoveTokenFrom(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testremoveTokenFrom(accounts[5], 1532892063,{from: accounts[0]});
  });
  it('Should execute test_mint(address,uint256) WHEN _to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyLeftGalleryToken.test_mint(accounts[2], 97,{from: accounts[0]});
  });
  it('Should fail test_mint(address,uint256) when NOT comply with: _to != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractProxyLeftGalleryToken.test_mint("0x0000000000000000000000000000000000000000", 97,{from: accounts[0]}),'revert');
  });
  it('Should execute test_burn(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.test_burn(accounts[4], 0,{from: accounts[0]});
  });
});
