...
contract("contractProxyLeftGalleryToken",(accounts)=>{
  ...
  it('Should execute test_setTokenURI(uint256,string)', async () => {
    let result = await contractProxyLeftGalleryToken.test_setTokenURI(1331007096, "file by ryan kuo, 2016, 100 editions",{from: accounts[0]});
  });
  it('Should execute testaddTokenTo(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testaddTokenTo(accounts[8], 31,{from: accounts[0]});
  });
  it('Should execute testremoveTokenFrom(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.testremoveTokenFrom(accounts[2], 3,{from: accounts[0]});
  });
  it('Should execute test_mint(address,uint256) WHEN _to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractProxyLeftGalleryToken.test_mint(accounts[2], 2014223715,{from: accounts[0]});
  });
  ...
  it('Should execute test_burn(address,uint256)', async () => {
    let result = await contractProxyLeftGalleryToken.test_burn(accounts[1], 5,{from: accounts[0]});
  });
});
