...
contract("LeftGalleryToken",(accounts)=>{
  ...
  it('Should execute tokenURI(uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenURI(26,{from: accounts[0]});
  });
  it('Should execute tokenOfOwnerByIndex(address,uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenOfOwnerByIndex(accounts[2], 1331007097,{from: accounts[0]});
  });
  ...
  it('Should execute tokenByIndex(uint256)', async () => {
    let result = await contractLeftGalleryToken.tokenByIndex(33,{from: accounts[0]});
  });
  it('Should execute createWork(address,string) WHEN msg.sender==owner,_to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractLeftGalleryToken.createWork(accounts[2], "monaco mix by rita vitorelli and harm van den dorpel, 2016, 100 editions",{from: accounts[0]});
  });
  it('Should fail createWork(address,string) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractLeftGalleryToken.createWork(accounts[2], "monaco mix by rita vitorelli and harm van den dorpel, 2016, 100 editions",{from: accounts[9]}),'revert');
  });
