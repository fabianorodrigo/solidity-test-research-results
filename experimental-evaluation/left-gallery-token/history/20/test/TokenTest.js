...
contract("Token",(accounts)=>{
  ...
  it('Should execute mint(address,string) WHEN msg.sender==owner,FunctionCall!=true,_to!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractToken.mint(accounts[7], "bedroom cuboro stacks by sean lockwood, 2016, 100 editions",{from: accounts[0]});
  });
  it('Should fail mint(address,string) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractToken.mint(accounts[7], "bedroom cuboro stacks by sean lockwood, 2016, 100 editions",{from: accounts[9]}),'revert');
  });
  it('Should fail mint(address,string) when NOT comply with: _to != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractToken.mint("0x0000000000000000000000000000000000000000", "bedroom cuboro stacks by sean lockwood, 2016, 100 editions",{from: accounts[0]}),'revert');
  });
  it('Should execute burn(uint256) WHEN msg.sender==owner', async () => {
    let result = await contractToken.burn(96,{from: accounts[0]});
  });
  it('Should fail burn(uint256) when NOT comply with: msg.sender == owner', async () => {
    let result = await truffleAssert.fails(contractToken.burn(96,{from: accounts[9]}),'revert');
  });
});
