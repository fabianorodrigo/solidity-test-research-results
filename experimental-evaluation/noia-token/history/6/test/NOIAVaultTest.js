...
contract("NOIAVault",(accounts)=>{
  ...
  it('Should execute initialize(address,uint256,address) WHEN beneficiary==0x0000000000000000000000000000000000000000,_beneficiary!=0x0000000000000000000000000000000000000000,_lockTill>block.timestamp', async () => {
    let result = await contractNOIAVault.initialize(accounts[1], (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+759, contractNOIAToken.address,{from: accounts[0]});
  });
  it('Should fail initialize(address,uint256,address) when NOT comply with: _beneficiary != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractNOIAVault.initialize("0x0000000000000000000000000000000000000000", (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+759, contractNOIAToken.address,{from: accounts[0]}),'revert');
  });
  it('Should fail initialize(address,uint256,address) when NOT comply with: _lockTill > (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp', async () => {
    let result = await truffleAssert.fails(contractNOIAVault.initialize(accounts[1], (await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp, contractNOIAToken.address,{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN balance>=amount', async () => {
    let result = await contractNOIAVault.recoverTokens(contractNOIAToken.address, accounts[8], 254,{from: accounts[0]});
  });
  it('Should execute tokensReceived(address,address,uint256)', async () => {
    let result = await contractNOIAVault.tokensReceived(accounts[0], accounts[6], 2014223714,{from: accounts[0]});
  });
});
