...
contract("contractProxyNOIAVaultFactory",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    if(trace) console.log('SUCESSO: NOIAVaultFactory.new(accounts[8],contractNOIAToken.address,{from:accounts[0]}');
      contractProxyNOIAVaultFactory = await ProxyNOIAVaultFactory.new(accounts[8],contractNOIAToken.address,{from:accounts[0]});
});
  
  it('Should execute testcreateClone(address)', async () => {
    let result = await contractProxyNOIAVaultFactory.testcreateClone(accounts[3],{from: accounts[0]});
  });
});
