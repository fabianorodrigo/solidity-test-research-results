...
contract("NOIAVaultFactory",(accounts)=>{
  ...
  it('Should execute createVault(address,uint256)', async () => {
    let result = await contractNOIAVaultFactory.createVault(accounts[4], 1532892064,{from: accounts[0]});
  });
  it('Should execute unlockableBalanceOf(address)', async () => {
    let result = await contractNOIAVaultFactory.unlockableBalanceOf(accounts[6],{from: accounts[0]});
  });
  it('Should execute totalBalanceOf(address)', async () => {
    let result = await contractNOIAVaultFactory.totalBalanceOf(accounts[9],{from: accounts[0]});
  });
  ...
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN balance>=amount', async () => {
    let result = await contractNOIAVaultFactory.recoverTokens(contractNOIAToken.address, accounts[6], 19,{from: accounts[0]});
  });
});
