...
contract("contractProxyNOIAVaultFactory",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    if(trace) console.log('SUCESSO: NOIAVaultFactory.new(accounts[4],contractTokenRecoverable.address,{from:accounts[0]}');
      contractProxyNOIAVaultFactory = await ProxyNOIAVaultFactory.new(accounts[4],contractTokenRecoverable.address,{from:accounts[0]});
});
  
  it('Should execute testcreateClone(address)', async () => {
    let result = await contractProxyNOIAVaultFactory.testcreateClone(accounts[1],{from: accounts[0]});
  });
});
