...
contract("TokenRecoverable",(accounts)=>{
  ...
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN msg.sender==_owner,balance>=amount', async () => {
    let result = await contractTokenRecoverable.recoverTokens(contractNOIAToken.address, accounts[8], 10001,{from: accounts[0]});
  });
  it('Should fail recoverTokens(IERC20,address,uint256) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractTokenRecoverable.recoverTokens(contractNOIAToken.address, accounts[8], 10001,{from: accounts[9]}),'revert');
  });
});
