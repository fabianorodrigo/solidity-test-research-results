...
contract("NOIAVaultFactory",(accounts)=>{
  ...
  it('Should execute createVault(address,uint256)', async () => {
    let result = await contractNOIAVaultFactory.createVault(accounts[3], 1532892063,{from: accounts[0]});
  });
  ...
  it('Should execute totalBalanceOf(address)', async () => {
    let result = await contractNOIAVaultFactory.totalBalanceOf(accounts[6],{from: accounts[0]});
  });
  it('Should execute getVaultsCount(address)', async () => {
    let result = await contractNOIAVaultFactory.getVaultsCount(accounts[8],{from: accounts[0]});
  });
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN balance>=amount', async () => {
    let result = await contractNOIAVaultFactory.recoverTokens(contractNOIAToken.address, accounts[2], 256,{from: accounts[0]});
  });
});
