...
contract("NOIAVaultFactory",(accounts)=>{
  ...
  it('Should execute createVault(address,uint256)', async () => {
    let result = await contractNOIAVaultFactory.createVault(accounts[5], 1532892063,{from: accounts[0]});
  });
  it('Should execute unlockableBalanceOf(address)', async () => {
    let result = await contractNOIAVaultFactory.unlockableBalanceOf(accounts[0],{from: accounts[0]});
  });
  it('Should execute totalBalanceOf(address)', async () => {
    let result = await contractNOIAVaultFactory.totalBalanceOf(accounts[0],{from: accounts[0]});
  });
  it('Should execute getVaultsCount(address)', async () => {
    let result = await contractNOIAVaultFactory.getVaultsCount(accounts[7],{from: accounts[0]});
  });
  it('Should execute recoverTokens(IERC20,address,uint256) WHEN balance>=amount', async () => {
    let result = await contractNOIAVaultFactory.recoverTokens(contractNOIAToken.address, accounts[5], 18,{from: accounts[0]});
  });
});
