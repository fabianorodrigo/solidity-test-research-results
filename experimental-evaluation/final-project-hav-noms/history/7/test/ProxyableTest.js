...
contract("Proxyable",(accounts)=>{
  ...
  it('Should execute setProxy(address) WHEN msg.sender==_owner', async () => {
    let result = await contractProxyable.setProxy(contractProxy.address,{from: accounts[0]});
  });
  it('Should fail setProxy(address) when NOT comply with: msg.sender == _owner', async () => {
    let result = await truffleAssert.fails(contractProxyable.setProxy(contractProxy.address,{from: accounts[9]}),'revert');
  });
  it('Should execute setMessageSender(address)', async () => {
    let result = await contractProxyable.setMessageSender(accounts[0],{from: accounts[0]});
  });
});
