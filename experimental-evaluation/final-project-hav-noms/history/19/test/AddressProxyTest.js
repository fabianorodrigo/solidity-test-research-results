...
contract("contractProxyAddress",(accounts)=>{
    let contractProxyAddress = null;
  let trace = false;
  ...
  beforeEach(async () => {
    ...
    if(trace) console.log('SUCESSO: TokenExchange.new(accounts[5],contractProxy.address,contractTokenExchangeState.address,"trade.totalPrice",{from:accounts[0]}');
      ProxyAddress.link('Address', contractAddress.address);
    contractProxyAddress = await ProxyAddress.new({ from: accounts[0] });
});
  
  it('Should execute testisContract(address)', async () => {
    let result = await contractProxyAddress.testisContract(accounts[7],{from: accounts[0]});
  });
});
