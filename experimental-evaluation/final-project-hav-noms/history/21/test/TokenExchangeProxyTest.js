...
const ProxyTokenExchange = artifacts.require("ProxyTokenExchange");

contract("contractProxyTokenExchange",(accounts)=>{
    let contractProxyTokenExchange = null;
  let trace = false;
  ...
  beforeEach(async () => {
    ...
    if(trace) console.log('SUCESSO: TokenExchange.new(accounts[4],contractProxy.address,contractTokenExchangeState.address,"\x19Ethereum Signed Message:\n32",{from:accounts[0]}');
      ProxyTokenExchange.link("SafeMath",contractSafeMath.address);
    ProxyTokenExchange.link("SafeDecimalMath",contractSafeDecimalMath.address);
    ProxyTokenExchange.link("Address",contractAddress.address);
    contractProxyTokenExchange = await ProxyTokenExchange.new({ from: accounts[0] });
});
  
  it('Should execute testremoveTradeListing(uint)', async () => {
    let result = await contractProxyTokenExchange.testremoveTradeListing(1532892062,{from: accounts[0]});
  });
});
