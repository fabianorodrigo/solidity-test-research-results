...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(95,accounts[2],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(95,accounts[2],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(1338,accounts[4],contractAtlantisToken.address,1532892063,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42+138,1,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(1338,accounts[4],contractAtlantisToken.address,1532892063,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+42+138,1,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(1337,accounts[1],contractAtlantisToken.address,500,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[4], 1532892064,{from: accounts[0]});
  });
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[4], 499,{from: accounts[0]}),'revert');
  });
});
