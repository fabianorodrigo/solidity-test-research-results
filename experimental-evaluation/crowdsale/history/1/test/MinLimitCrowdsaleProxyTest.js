...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(1,accounts[6],contractAtlantisToken.address,29,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+84,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+84+603,3,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(2,accounts[1],contractAtlantisToken.address,9,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[1], 1532892063,{from: accounts[0]});
  });
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[1], 8,{from: accounts[0]}),'revert');
  });
});
