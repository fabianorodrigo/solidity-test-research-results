...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractCrowdsale = await Crowdsale.new(1,accounts[4],contractAtlantisToken.address,{from:accounts[0]});
    if(trace) console.log('SUCESSO: Crowdsale.new(1,accounts[4],contractAtlantisToken.address,{from:accounts[0]}');
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(10,accounts[6],contractAtlantisToken.address,1336,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+167,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+167+95,29,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(10,accounts[6],contractAtlantisToken.address,1336,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+167,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+167+95,29,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(254,accounts[8],contractAtlantisToken.address,500,{from:accounts[0]});
});
