...
contract("contractProxyMinLimitCrowdsale",(accounts)=>{
  ...
  beforeEach(async () => {
    ...
    contractAtlantisCrowdsale = await AtlantisCrowdsale.new(254,accounts[7],contractAtlantisToken.address,6,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+247,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+247+898,64,{from:accounts[0]});
    if(trace) console.log('SUCESSO: AtlantisCrowdsale.new(254,accounts[7],contractAtlantisToken.address,6,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+247,(await web3.eth.getBlock(await web3.eth.getBlockNumber())).timestamp+247+898,64,{from:accounts[0]}');
      contractProxyMinLimitCrowdsale = await ProxyMinLimitCrowdsale.new(97,accounts[7],contractAtlantisToken.address,10000,{from:accounts[0]});
});
  ...
  it('Should execute test_preValidatePurchase(address,uint256) WHEN beneficiary!=0x0000000000000000000000000000000000000000,weiAmount!=0,weiAmount>=_minLimit', async () => {
    let result = await contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[2], 1532892064,{from: accounts[0]});
  });
  ...
  it('Should fail test_preValidatePurchase(address,uint256) when NOT comply with: weiAmount >= _minLimit', async () => {
    let result = await truffleAssert.fails(contractProxyMinLimitCrowdsale.test_preValidatePurchase(accounts[2], 9999,{from: accounts[0]}),'revert');
  });
});
