...
contract("GolemNetworkToken",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN _value<=0', async () => {
    let result = await contractGolemNetworkToken.transfer(accounts[7], 0,{from: accounts[0]});
  });
  ...
  it('Should execute migrate(uint256) WHEN migrationAgent!=0x0000000000000000000000000000000000000000,_value!=0,_value<=balances', async () => {
    await contractGolemNetworkToken.setMigrationAgent(accounts[7],{from:accounts[1]});
  ...
  it('Should execute setMigrationMaster(address) WHEN msg.sender==migrationMaster,_master!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractGolemNetworkToken.setMigrationMaster(accounts[5],{from: accounts[1]});
  });
  it('Should fail setMigrationMaster(address) when NOT comply with: msg.sender == migrationMaster', async () => {
    let result = await truffleAssert.fails(contractGolemNetworkToken.setMigrationMaster(accounts[5],{from: accounts[9]}),'revert');
  });
  ...
  it('Should execute create() WHEN funding==true,(await web3.eth.getBlockNumber())>=fundingStartBlock,(await web3.eth.getBlockNumber())<=fundingEndBlock,msg.value!=0', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[5],accounts[3],(await web3.eth.getBlockNumber())+304,(await web3.eth.getBlockNumber())+304+615,{from:accounts[0]});
    let result = await localGolemNetworkToken.create({from: accounts[0],value:99});
  });
  it('Should fail create() when NOT comply with: funding == true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[5],accounts[3],(await web3.eth.getBlockNumber())+304,(await web3.eth.getBlockNumber())+304+615,{from:accounts[0]});
    await localGolemNetworkToken.finalize({from:accounts[0],value:99});
    let result = await truffleAssert.fails(localGolemNetworkToken.create({from: accounts[0],value:99}),'revert');
  });
  ...
  it('Should execute refund() WHEN funding==true,(await web3.eth.getBlockNumber())>fundingEndBlock,totalTokens<tokenCreationMin,gntValue!=0,FunctionCall==true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[6],accounts[6],(await web3.eth.getBlockNumber())+189,(await web3.eth.getBlockNumber())+189+329,{from:accounts[0]});
