...
contract("GolemNetworkToken",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN senderBalance>=_value,_value>0', async () => {
    let result = await contractGolemNetworkToken.transfer(accounts[3], 15,{from: accounts[0]});
  });
  ...
  it('Should execute migrate(uint256) WHEN migrationAgent!=0x0000000000000000000000000000000000000000,_value!=0,_value<=balances', async () => {
    ...
    let result = await contractGolemNetworkToken.migrate(3,{from: accounts[7]});
  });
  it('Should fail migrate(uint256) when NOT comply with: migrationAgent != 0x0000000000000000000000000000000000000000', async () => {
    ...
    let result = await truffleAssert.fails(contractGolemNetworkToken.migrate(3,{from: accounts[7]}),'revert');
  });
  ...
  it('Should execute setMigrationMaster(address) WHEN msg.sender==migrationMaster,_master!=0x0000000000000000000000000000000000000000', async () => {
    let result = await contractGolemNetworkToken.setMigrationMaster(accounts[4],{from: accounts[7]});
  });
  ...
  it('Should execute create() WHEN funding==true,(await web3.eth.getBlockNumber())>=fundingStartBlock,(await web3.eth.getBlockNumber())<=fundingEndBlock,msg.value!=0', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[8],accounts[6],(await web3.eth.getBlockNumber())+572,(await web3.eth.getBlockNumber())+572+901,{from:accounts[0]});
    let result = await localGolemNetworkToken.create({from: accounts[0],value:19});
  });
  it('Should fail create() when NOT comply with: funding == true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[8],accounts[6],(await web3.eth.getBlockNumber())+572,(await web3.eth.getBlockNumber())+572+901,{from:accounts[0]});
    await localGolemNetworkToken.finalize({from:accounts[0],value:19});
    let result = await truffleAssert.fails(localGolemNetworkToken.create({from: accounts[0],value:19}),'revert');
  });
  ...
  it('Should execute refund() WHEN funding==true,(await web3.eth.getBlockNumber())>fundingEndBlock,totalTokens<tokenCreationMin,gntValue!=0,FunctionCall==true', async () => {
    let localGolemNetworkToken = await GolemNetworkToken.new(accounts[7],accounts[2],(await web3.eth.getBlockNumber())+606,(await web3.eth.getBlockNumber())+606+907,{from:accounts[0]});
