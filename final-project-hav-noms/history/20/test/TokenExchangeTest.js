...
contract("TokenExchange",(accounts)=>{
  ...
  it('Should execute getTrade(uint)', async () => {
    let result = await contractTokenExchange.getTrade(32,{from: accounts[0]});
  });
  it('Should execute createTradeListing(uint,uint,address) WHEN paused!=true,amount>0,ethRate>0', async () => {
    let result = await contractTokenExchange.createTradeListing(58, 58, contractShartCoin.address,{from: accounts[0]});
  });
  it('Should fail createTradeListing(uint,uint,address) when NOT comply with: paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractTokenExchange.createTradeListing(58, 58, contractShartCoin.address,{from: accounts[0]}),'revert');
  });
  it('Should fail createTradeListing(uint,uint,address) when NOT comply with: amount > 0', async () => {
    let result = await truffleAssert.fails(contractTokenExchange.createTradeListing(0, 58, contractShartCoin.address,{from: accounts[0]}),'revert');
  });
  it('Should fail createTradeListing(uint,uint,address) when NOT comply with: ethRate > 0', async () => {
    let result = await truffleAssert.fails(contractTokenExchange.createTradeListing(58, 0, contractShartCoin.address,{from: accounts[0]}),'revert');
  });
  it('Should execute withdrawMyDepositedTokens(uint) WHEN paused!=true', async () => {
    let result = await contractTokenExchange.withdrawMyDepositedTokens(1532892063,{from: accounts[0]});
  });
  it('Should fail withdrawMyDepositedTokens(uint) when NOT comply with: paused != true', async () => {
    await contractPausable.transferOwnership(accounts[8],{from:accounts[0]});
    let result = await truffleAssert.fails(contractTokenExchange.withdrawMyDepositedTokens(1532892063,{from: accounts[0]}),'revert');
  });
  it('Should execute exchangeEtherForTokens(uint) WHEN paused!=true,reentrancyLock!=true', async () => {
    let result = await contractTokenExchange.exchangeEtherForTokens(10,{from: accounts[0]});
  });
  it('Should fail exchangeEtherForTokens(uint) when NOT comply with: paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractTokenExchange.exchangeEtherForTokens(10,{from: accounts[0]}),'revert');
  });
