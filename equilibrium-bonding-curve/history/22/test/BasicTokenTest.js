...
contract("BasicToken",(accounts)=>{
  ...
  it('Should execute transfer(address,uint256) WHEN _to!=0x0000000000000000000000000000000000000000,_value<=balances', async () => {
    let result = await contractBasicToken.transfer(accounts[5], 100,{from: accounts[0]});
  });
  it('Should fail transfer(address,uint256) when NOT comply with: _to != 0x0000000000000000000000000000000000000000', async () => {
    let result = await truffleAssert.fails(contractBasicToken.transfer("0x0000000000000000000000000000000000000000", 100,{from: accounts[0]}),'revert');
  });
  it('Should execute balanceOf(address)', async () => {
    let result = await contractBasicToken.balanceOf(accounts[3],{from: accounts[0]});
  });
});
