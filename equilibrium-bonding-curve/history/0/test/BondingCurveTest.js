...
contract("BondingCurve",(accounts)=>{
  ...
  it('Should execute buyTokens(uint256,uint256) WHEN supply>0', async () => {
    let result = await contractBondingCurve.buyTokens(1, 2,{from: accounts[0]});
  });
  it('Should execute buyTokens(uint256,uint256) WHEN supply<=0', async () => {
    await contractBondingCurve.sellTokens(128,{from:accounts[0]});
    let result = await contractBondingCurve.buyTokens(257, 1,{from: accounts[0]});
  });
  it('Should execute buyTokens(uint256,uint256) WHEN supply==0', async () => {
    await contractBondingCurve.sellTokens(129,{from:accounts[0]});
    let result = await contractBondingCurve.buyTokens(1, 255,{from: accounts[0]});
  });
  it('Should execute buyTokens(uint256,uint256) WHEN supply!=0', async () => {
    let result = await contractBondingCurve.buyTokens(127, 129,{from: accounts[0]});
  });
