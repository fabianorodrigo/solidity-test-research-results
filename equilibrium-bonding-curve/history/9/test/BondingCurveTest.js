...
contract("BondingCurve",(accounts)=>{
  ...
  it('Should execute buyTokens(uint256,uint256) WHEN supply>0', async () => {
    let result = await contractBondingCurve.buyTokens(128, 99,{from: accounts[0]});
  });
  it('Should execute buyTokens(uint256,uint256) WHEN supply<=0', async () => {
    await contractBondingCurve.sellTokens(10001,{from:accounts[0]});
    let result = await contractBondingCurve.buyTokens(2, 0,{from: accounts[0]});
  });
  it('Should execute buyTokens(uint256,uint256) WHEN supply==0', async () => {
  ...
  it('Should execute queryNextPrice() WHEN supply==0', async () => {
    await contractBondingCurve.buyTokens(10001,129,{from:accounts[0]});
    let result = await contractBondingCurve.queryNextPrice({from: accounts[0]});
  });
  it('Should execute queryNextPrice() WHEN supply!=0', async () => {
    let result = await contractBondingCurve.queryNextPrice({from: accounts[0]});
