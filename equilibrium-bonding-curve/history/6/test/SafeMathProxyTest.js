...
contract("contractProxySafeMath",(accounts)=>{
  ...
  it('Should execute testmul(uint256,uint256) WHEN a==0', async () => {
    let result = await contractProxySafeMath.testmul(0, 128,{from: accounts[0]});
  });
  it('Should fail testmul(uint256,uint256) when NOT comply with: a == 0', async () => {
    let result = await truffleAssert.fails(contractProxySafeMath.testmul(1, 128,{from: accounts[0]}),'revert');
  });
  it('Should execute testdiv(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testdiv(2, 3,{from: accounts[0]});
  });
  it('Should execute testsub(uint256,uint256) WHEN b<=a', async () => {
    let result = await contractProxySafeMath.testsub(9999, 99,{from: accounts[0]});
  });
  ...
  it('Should execute testadd(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testadd(2, 99,{from: accounts[0]});
  });
});
