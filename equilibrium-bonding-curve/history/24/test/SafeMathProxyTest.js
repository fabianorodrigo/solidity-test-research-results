...
contract("contractProxySafeMath",(accounts)=>{
  ...
  it('Should execute testmul(uint256,uint256) WHEN a==0', async () => {
    let result = await contractProxySafeMath.testmul(0, 256,{from: accounts[0]});
  });
  it('Should fail testmul(uint256,uint256) when NOT comply with: a == 0', async () => {
    let result = await truffleAssert.fails(contractProxySafeMath.testmul(1, 256,{from: accounts[0]}),'revert');
  });
  it('Should execute testdiv(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testdiv(1, 257,{from: accounts[0]});
  });
  it('Should execute testsub(uint256,uint256) WHEN b<=a', async () => {
    let result = await contractProxySafeMath.testsub(129, 0,{from: accounts[0]});
  });
  it('Should fail testsub(uint256,uint256) when NOT comply with: b <= a', async () => {
    let result = await truffleAssert.fails(contractProxySafeMath.testsub(129, 130,{from: accounts[0]}),'revert');
  });
  it('Should execute testadd(uint256,uint256)', async () => {
    let result = await contractProxySafeMath.testadd(0, 2,{from: accounts[0]});
  });
});
