...
contract("Pony",(accounts)=>{
  ...
  it('Should execute tokenFallback(address,uint256,bytes) WHEN value>=PLAY_COST', async () => {
    let result = await contractPony.tokenFallback(accounts[0], 759, [11,106,190,33,236,209,161,24,217,213,53,61,193,183,153,177,234,90,188,208,1,180,52,50,212,58,226,35,114,150,140,43],{from: accounts[0]});
  });
  it('Should fail tokenFallback(address,uint256,bytes) when NOT comply with: value >= PLAY_COST', async () => {
    let result = await truffleAssert.fails(contractPony.tokenFallback(accounts[0], 757, [11,106,190,33,236,209,161,24,217,213,53,61,193,183,153,177,234,90,188,208,1,180,52,50,212,58,226,35,114,150,140,43],{from: accounts[0]}),'revert');
  });
