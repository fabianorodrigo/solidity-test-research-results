...
contract("Pony",(accounts)=>{
  ...
  it('Should execute tokenFallback(address,uint256,bytes) WHEN value>=PLAY_COST', async () => {
    let result = await contractPony.tokenFallback(accounts[3], 759, [64,89,92,221,217,1,111,215,13,29,245,232,9,13,158,181,138,7,232,226,250,246,87,3,95,42,18,182,93,22,41,156],{from: accounts[0]});
  });
  it('Should fail tokenFallback(address,uint256,bytes) when NOT comply with: value >= PLAY_COST', async () => {
    let result = await truffleAssert.fails(contractPony.tokenFallback(accounts[3], 757, [64,89,92,221,217,1,111,215,13,29,245,232,9,13,158,181,138,7,232,226,250,246,87,3,95,42,18,182,93,22,41,156],{from: accounts[0]}),'revert');
  });
