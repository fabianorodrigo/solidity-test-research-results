...
contract("contractProxyRelevantToken",(accounts)=>{
  ...
  it('Should execute testpreMintTokens(uint256)', async () => {
    let result = await contractProxyRelevantToken.testpreMintTokens(53,{from: accounts[0]});
  });
  it('Should execute testnewTokensForConstantPhase(uint256,uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForConstantPhase(59, 26,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound==0', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(85,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound!=0', async () => {
    await contractProxyRelevantToken.releaseTokens({from:accounts[0]});
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(83,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN _roundsPassed<100000', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(44,{from: accounts[0]});
  });
  ...
  it('Should execute testsplitRewards(uint256)', async () => {
    let result = await contractProxyRelevantToken.testsplitRewards(982081,{from: accounts[0]});
  });
