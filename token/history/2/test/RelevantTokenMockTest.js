...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("0.3",37,"IsLibrary","0.3",accounts[7],69,122,117,110,36,62,101,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(1, 26, 10001,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("0.3",37,"IsLibrary","0.3",accounts[7],69,122,117,110,36,62,101,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(1, 26, 10001,{from: accounts[9]}),'revert');
  });
