...
contract("contractProxyRelevantToken",(accounts)=>{
  ...
  it('Should execute testpreMintTokens(uint256)', async () => {
    let result = await contractProxyRelevantToken.testpreMintTokens(32,{from: accounts[0]});
  });
  it('Should execute testnewTokensForConstantPhase(uint256,uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForConstantPhase(257, 96,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound==0', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(1000000,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound!=0', async () => {
  ...
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN _roundsPassed<100000', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(51,{from: accounts[0]});
  });
  ...
  it('Should execute testnewTokensForCrossingConst(uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForCrossingConst(32735,{from: accounts[0]});
  });
  it('Should execute testsplitRewards(uint256)', async () => {
    let result = await contractProxyRelevantToken.testsplitRewards(3,{from: accounts[0]});
  });
