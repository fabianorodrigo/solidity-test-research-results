...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setRoundNum(uint256)', async () => {
    let result = await contractRelevantTokenMock.setRoundNum(90,{from: accounts[0]});
  });
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("RevertWithReason",55,"0.3","IsLibrary",accounts[8],103,52,104,90,55,39,121,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(63, 1337, 90,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("RevertWithReason",55,"0.3","IsLibrary",accounts[8],103,52,104,90,55,39,121,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(63, 1337, 90,{from: accounts[9]}),'revert');
  });
