...
contract("contractProxyPower",(accounts)=>{
  ...
  it('Should execute testpower(uint256,uint256,uint32,uint32)', async () => {
    let result = await contractProxyPower.testpower(3, 87, 38, 1000000,{from: accounts[0]});
  });
  it('Should execute testln(uint256,uint256) WHEN _numerator<=MAX_NUM', async () => {
    let result = await contractProxyPower.testln(2014223714, 37,{from: accounts[0]});
  });
  it('Should fail testln(uint256,uint256) when NOT comply with: _numerator <= MAX_NUM', async () => {
    let result = await truffleAssert.fails(contractProxyPower.testln(new web3.utils.BN('6.80564733841877e+38'), 37,{from: accounts[0]}),'revert');
  });
  it('Should execute testfloorLog2(uint256)', async () => {
    let result = await contractProxyPower.testfloorLog2(35,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray>=_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(1532892064,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray<_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(982080,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray>=_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(9,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray<_x,maxExpArray<_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(55,{from: accounts[0]});
  });
  it('Should execute testfixedExp(uint256,uint8)', async () => {
    let result = await contractProxyPower.testfixedExp(1338, 127,{from: accounts[0]});
  });
});
