...
contract("contractProxyMathUtils",(accounts)=>{
  ...
  it('Should execute testvalidPerc(uint256)', async () => {
    let result = await contractProxyMathUtils.testvalidPerc(124,{from: accounts[0]});
  });
  it('Should execute testpercOf0(uint256,uint256,uint256) WHEN PERC_DIVISOR>0', async () => {
    let result = await contractProxyMathUtils.testpercOf0(58, 0, 9999,{from: accounts[0]});
  });
  it('Should execute testpercOf1(uint256,uint256) WHEN PERC_DIVISOR>0', async () => {
    let result = await contractProxyMathUtils.testpercOf1(1337, 111,{from: accounts[0]});
  });
  it('Should execute testpercPoints(uint256,uint256) WHEN _fracDenom>0', async () => {
    let result = await contractProxyMathUtils.testpercPoints(1338, 1,{from: accounts[0]});
  });
  it('Should fail testpercPoints(uint256,uint256) when NOT comply with: _fracDenom > 0', async () => {
    let result = await truffleAssert.fails(contractProxyMathUtils.testpercPoints(1338, 0,{from: accounts[0]}),'revert');
  });
});
