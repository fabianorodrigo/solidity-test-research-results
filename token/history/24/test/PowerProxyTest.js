...
contract("contractProxyPower",(accounts)=>{
  ...
  it('Should execute testpower(uint256,uint256,uint32,uint32)', async () => {
    let result = await contractProxyPower.testpower(1000000, 1, 999999, 2014223716,{from: accounts[0]});
  });
  it('Should execute testln(uint256,uint256) WHEN _numerator<=MAX_NUM', async () => {
    let result = await contractProxyPower.testln(100001, 47,{from: accounts[0]});
  });
  it('Should fail testln(uint256,uint256) when NOT comply with: _numerator <= MAX_NUM', async () => {
    let result = await truffleAssert.fails(contractProxyPower.testln(new web3.utils.BN('6.80564733841877e+38'), 47,{from: accounts[0]}),'revert');
  });
  it('Should execute testfloorLog2(uint256)', async () => {
    let result = await contractProxyPower.testfloorLog2(4,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray>=_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(73,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray<_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(46,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray>=_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(1337,{from: accounts[0]});
  });
  it('Should execute testfindPositionInMaxExpArray(uint256) WHEN maxExpArray<_x,maxExpArray<_x', async () => {
    let result = await contractProxyPower.testfindPositionInMaxExpArray(982080,{from: accounts[0]});
  });
  it('Should execute testfixedExp(uint256,uint8)', async () => {
    let result = await contractProxyPower.testfixedExp(126, 37,{from: accounts[0]});
  });
});
