...
contract("contractProxyPower",(accounts)=>{
  ...
  it('Should execute testpower(uint256,uint256,uint32,uint32)', async () => {
    let result = await contractProxyPower.testpower(1338, 102, 66, 1336,{from: accounts[0]});
  });
  it('Should execute testln(uint256,uint256) WHEN _numerator<=MAX_NUM', async () => {
    let result = await contractProxyPower.testln(33, 4,{from: accounts[0]});
  });
  it('Should fail testln(uint256,uint256) when NOT comply with: _numerator <= MAX_NUM', async () => {
    let result = await truffleAssert.fails(contractProxyPower.testln(new web3.utils.BN('6.80564733841877e+38'), 4,{from: accounts[0]}),'revert');
  });
  ...
  it('Should execute testfixedExp(uint256,uint8)', async () => {
    let result = await contractProxyPower.testfixedExp(79, 98,{from: accounts[0]});
  });
});
