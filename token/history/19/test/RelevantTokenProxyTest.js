...
contract("contractProxyRelevantToken",(accounts)=>{
  ...
  it('Should execute testpreMintTokens(uint256)', async () => {
    let result = await contractProxyRelevantToken.testpreMintTokens(121,{from: accounts[0]});
  });
  it('Should execute testnewTokensForConstantPhase(uint256,uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForConstantPhase(6, 36,{from: accounts[0]});
  });
  ...
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN _roundsPassed<100000', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(0,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN _roundsPassed>=100000,lastRound!=0', async () => {
    await contractProxyRelevantToken.releaseTokens({from:accounts[0]});
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(2014223714,{from: accounts[0]});
  });
  ...
  it('Should execute testnewTokensForCrossingConst(uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForCrossingConst(71,{from: accounts[0]});
  });
  it('Should execute testsplitRewards(uint256)', async () => {
    let result = await contractProxyRelevantToken.testsplitRewards(55,{from: accounts[0]});
  });
