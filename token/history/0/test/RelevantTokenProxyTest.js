...
contract("contractProxyRelevantToken",(accounts)=>{
  ...
  it('Should execute testpreMintTokens(uint256)', async () => {
    let result = await contractProxyRelevantToken.testpreMintTokens(128,{from: accounts[0]});
  });
  it('Should execute testnewTokensForConstantPhase(uint256,uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForConstantPhase(32735, 11,{from: accounts[0]});
  });
  ...
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound!=0', async () => {
    ...
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(53,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN _roundsPassed<100000', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(9,{from: accounts[0]});
  });
  ...
  it('Should execute testnewTokensForCrossingConst(uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForCrossingConst(55,{from: accounts[0]});
  });
  it('Should execute testsplitRewards(uint256)', async () => {
    let result = await contractProxyRelevantToken.testsplitRewards(78,{from: accounts[0]});
  });
