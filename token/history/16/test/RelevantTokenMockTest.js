...
contract("RelevantTokenMock",(accounts)=>{
  ...
  it('Should execute setLastRound(uint256,uint256,uint256) WHEN msg.sender==_owner,_roundNum<currentRound', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("RevertWithReason",101,"PayableExample","PayableExample",accounts[1],75,86,63,1,90,33,982080,{from:accounts[0]});
    let result = await contractRelevantTokenMock.setLastRound(65, 49, 57,{from: accounts[0]});
  });
  it('Should fail setLastRound(uint256,uint256,uint256) when NOT comply with: msg.sender == _owner', async () => {
    await contractRelevantTokenMock.methods["initialize(string,uint8,string,string,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"]("RevertWithReason",101,"PayableExample","PayableExample",accounts[1],75,86,63,1,90,33,982080,{from:accounts[0]});
    let result = await truffleAssert.fails(contractRelevantTokenMock.setLastRound(65, 49, 57,{from: accounts[9]}),'revert');
  });
