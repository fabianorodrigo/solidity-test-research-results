...
contract("contractProxyRelevantToken",(accounts)=>{
  ...
  it('Should execute testpreMintTokens(uint256)', async () => {
    let result = await contractProxyRelevantToken.testpreMintTokens(72,{from: accounts[0]});
  });
  it('Should execute testnewTokensForConstantPhase(uint256,uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForConstantPhase(100000, 85,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound==0', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(42,{from: accounts[0]});
  });
  it('Should execute testnewTokensForDecayPhase(uint256) WHEN lastRound!=0', async () => {
    await contractProxyRelevantToken.releaseTokens({from:accounts[0]});
    let result = await contractProxyRelevantToken.testnewTokensForDecayPhase(6,{from: accounts[0]});
  });
  ...
  it('Should execute testnewTokensForCrossingConst(uint256)', async () => {
    let result = await contractProxyRelevantToken.testnewTokensForCrossingConst(121,{from: accounts[0]});
  });
  it('Should execute testsplitRewards(uint256)', async () => {
    let result = await contractProxyRelevantToken.testsplitRewards(90,{from: accounts[0]});
  });
