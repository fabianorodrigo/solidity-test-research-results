...
contract("SupplyChain",(accounts)=>{
  ...
  it('Should execute addItem(string,uint)', async () => {
    let result = await contractSupplyChain.addItem("o58dgm", 2,{from: accounts[0]});
  });
  it('Should execute buyItem(uint) WHEN msg.value>=sku', async () => {
    let result = await contractSupplyChain.buyItem(257,{from: accounts[0],value:1336});
  });
  ...
  it('Should execute shipItem(uint)', async () => {
    let result = await contractSupplyChain.shipItem(5,{from: accounts[0]});
  });
