...
contract("BulkTransfer",(accounts)=>{
  ...
  it('Should execute bulkTransfer(address,address[],uint256[]) WHEN _paused!=true,destinations.length==amounts.length', async () => {
    let result = await contractBulkTransfer.bulkTransfer(contractSimpleToken.address, [accounts[5],accounts[9]], [255,0],{from: accounts[0]});
  });
  it('Should fail bulkTransfer(address,address[],uint256[]) when NOT comply with: _paused != true', async () => {
    ...
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransfer(contractSimpleToken.address, [accounts[5],accounts[9]], [255,0],{from: accounts[0]}),'revert');
  });
  it('Should fail bulkTransfer(address,address[],uint256[]) when NOT comply with: destinations.length == amounts.length', async () => {
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransfer(contractSimpleToken.address, [accounts[5],accounts[4],accounts[0]], [255,0],{from: accounts[0]}),'revert');
  });
  it('Should execute bulkTransferEther(address[],uint256[]) WHEN _paused!=true,destinations.length==amounts.length', async () => {
    let result = await contractBulkTransfer.bulkTransferEther([accounts[6],accounts[6]], [66,28],{from: accounts[0]});
  });
  it('Should fail bulkTransferEther(address[],uint256[]) when NOT comply with: _paused != true', async () => {
    await contractBulkTransfer.pause({from:accounts[0]});
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransferEther([accounts[6],accounts[6]], [66,28],{from: accounts[0]}),'revert');
  });
  it('Should fail bulkTransferEther(address[],uint256[]) when NOT comply with: destinations.length == amounts.length', async () => {
    let result = await truffleAssert.fails(contractBulkTransfer.bulkTransferEther([accounts[1],accounts[4],accounts[9]], [66,28],{from: accounts[0]}),'revert');
  });
});
