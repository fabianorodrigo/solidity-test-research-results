...
contract("CustomOwnable",(accounts)=>{
  ...
  it('Should execute reassignOwner(address) WHEN msg.sender==_trustee,newOwner!=0x0000000000000000000000000000000000000000', async () => {
    await contractCustomOwnable.assignTrustee(accounts[3],{from:accounts[0]});
    let result = await contractCustomOwnable.reassignOwner(accounts[7],{from: accounts[3]});
  });
  ...
  it('Should fail reassignOwner(address) when NOT comply with: newOwner != 0x0000000000000000000000000000000000000000', async () => {
    await contractCustomOwnable.assignTrustee(accounts[3],{from:accounts[0]});
    let result = await truffleAssert.fails(contractCustomOwnable.reassignOwner("0x0000000000000000000000000000000000000000",{from: accounts[3]}),'revert');
  });
