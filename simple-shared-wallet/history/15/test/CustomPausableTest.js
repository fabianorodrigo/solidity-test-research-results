...
contract("CustomPausable",(accounts)=>{
  let trace = false;
  ...
  it('Should execute pause() WHEN _paused!=true', async () => {
    let result = await contractCustomPausable.pause({from: accounts[0]});
  });
  it('Should execute unpause() WHEN _paused==true', async () => {
    await contractCustomPausable.pause({from:accounts[0]});
    let result = await contractCustomPausable.unpause({from: accounts[0]});
  });
  it('Should fail unpause() when NOT comply with: _paused == true', async () => {
    let result = await truffleAssert.fails(contractCustomPausable.unpause({from: accounts[0]}),'revert');
  });
  it('Should execute isPaused()', async () => {
    let result = await contractCustomPausable.isPaused({from: accounts[0]});
  });
});
