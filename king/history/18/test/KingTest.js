...
contract("King",(accounts)=>{
  ...
  it('Should execute tokenFallback(address,uint256,bytes) WHEN value>=PLAY_COST', async () => {
    let result = await contractKing.tokenFallback(accounts[6], 758, [47,38,140,178,195,69,16,80,203,214,221,100,244,237,35,253,159,50,137,170,228,224,162,23,236,67,205,84,122,163,225,123],{from: accounts[0]});
  });
  it('Should fail tokenFallback(address,uint256,bytes) when NOT comply with: value >= PLAY_COST', async () => {
    let result = await truffleAssert.fails(contractKing.tokenFallback(accounts[6], 757, [47,38,140,178,195,69,16,80,203,214,221,100,244,237,35,253,159,50,137,170,228,224,162,23,236,67,205,84,122,163,225,123],{from: accounts[0]}),'revert');
  });
